function [ final_acc, final_itr ] = erp_performance(CNT, initParam)
    ...
    for add=1:60:length(dat.t)
        dat_char_all.x(:,:,:,opt.init_speller_length)=dat.x(:,[add:add+59],:);
        dat_char_all.t(:,:,:,opt.init_speller_length)=dat.t(:,[add:add+59]);
