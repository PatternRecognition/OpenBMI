function varargout = select_channel_ver(varargin)
...
j = size(chan,2);
for i = 1: size(aaa.clab, 2)
    [row, col]= find(strcmp(chan,aaa.clab{i}));
    if(isempty(row)&&isempty(col))
        a{19, j} = sprintf('%s (%d)',aaa.clab{i},i);
        j = j-1;
    else
        a{row, col} = sprintf('%s (%d)',aaa.clab{i},i);
    end
end
