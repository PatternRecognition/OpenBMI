function [ output_args ] = p300_online_revised(fig,txt, varargin)
...
% 2018. 05. 11. revised by Hong-Kyung Kim
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%
% Init
...
if isnumeric(opt.channel)
    channel_idx = false(1, length(state.chan_sel));
    channel_idx(opt.channel) = true;
else
    channel_idx = ismember(state.chan_sel, opt.channel);
end
        ...
        if nc <= i
            ...
            if ~isempty(IV)
                if IV(end)<length(EEG_data)
                    ...
                    dat=func_featureExtraction(dat,{'feature','erpmean';'nMeans',nFeature}); %% revised 2017.07.28 (oyeon)
                    ...
                    infor = sprintf('trial:%s,  sequence: %d, run: %d', nChar, j, nc);
                    ...
                    [~, maxIndex] = min(y2(:));  %save best for eog
                    ...
                    for ii=1  %-- just get the first target char (1:3 you will take to the third low target)
                        ...
                        p_1(ii)= find(spell_char==temp2{row,col});  % spell_char((6-1) * 6 + 6)
                    end
                    %% Save
                    if nc==12*j
                        if j==10
                            ...
                            t_char =temp2{row1,col1}; % spell_char((6-1) * 6 + 6)
