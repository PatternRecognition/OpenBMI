...
class DeepSleepNet(DeepFeatureNet):
    ...
    def build_model(self, input_var):
        ...
        with tf.variable_scope(name) as scope:
            # fw_lstm_cell = tf.compat.v1.nn.rnn_cell.LSTMCell(hidden_size,
            #                                        use_peepholes=True,
            #                                        state_is_tuple=True)
            # bw_lstm_cell = tf.compat.v1.nn.rnn_cell.LSTMCell(hidden_size,
            #                                        use_peepholes=True,
            #                                        state_is_tuple=True)
            if self.use_dropout_sequence:
                keep_prob = 0.5 if self.is_train else 1.0
                fw_lstm_cell1 = tf.compat.v1.nn.rnn_cell.LSTMCell(hidden_size, use_peepholes=True, state_is_tuple=True)
                fw_lstm_cell1 = tf.compat.v1.nn.rnn_cell.DropoutWrapper(fw_lstm_cell1, output_keep_prob=keep_prob)
                fw_lstm_cell2 = tf.compat.v1.nn.rnn_cell.LSTMCell(hidden_size, use_peepholes=True, state_is_tuple=True)
                fw_lstm_cell2 = tf.compat.v1.nn.rnn_cell.DropoutWrapper(fw_lstm_cell2, output_keep_prob=keep_prob)

                bw_lstm_cell1 = tf.compat.v1.nn.rnn_cell.LSTMCell(hidden_size, use_peepholes=True, state_is_tuple=True)
                bw_lstm_cell1 = tf.compat.v1.nn.rnn_cell.DropoutWrapper(bw_lstm_cell1, output_keep_prob=keep_prob)
                bw_lstm_cell2 = tf.compat.v1.nn.rnn_cell.LSTMCell(hidden_size, use_peepholes=True, state_is_tuple=True)
                bw_lstm_cell2 = tf.compat.v1.nn.rnn_cell.DropoutWrapper(bw_lstm_cell2, output_keep_prob=keep_prob)

            # fw_cell = tf.compat.v1.nn.rnn_cell.MultiRNNCell([fw_lstm_cell] * self.n_rnn_layers,
            #                                       state_is_tuple=True)
            # bw_cell = tf.compat.v1.nn.rnn_cell.MultiRNNCell([bw_lstm_cell] * self.n_rnn_layers,
            #                                       state_is_tuple=True)

            fw_cell = tf.compat.v1.nn.rnn_cell.MultiRNNCell([fw_lstm_cell1, fw_lstm_cell2], state_is_tuple=True)
            bw_cell = tf.compat.v1.nn.rnn_cell.MultiRNNCell([bw_lstm_cell1, bw_lstm_cell2], state_is_tuple=True)
