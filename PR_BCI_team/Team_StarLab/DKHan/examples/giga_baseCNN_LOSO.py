...
class GigaDataset_session_wise(Dataset):
    ...
    def __init__(self,transform=None,istrain = True, subj=54, sess=0):
        ...
        if istrain:
            x_data = np.delete(x_data,np.s_[10800*sess:10800*sess+10800],0) #sess 선택
...
def main():
    # Training settings
    parser = argparse.ArgumentParser(description='PyTorch MI Example')
    ...
    f = open("base_cnn1" + datetime.today().strftime("%m_%d_%H_%M") + ".txt", 'w')

    for subj in range(26,54):
        dataset = GigaDataset_LOSO(istrain = True, subj=subj)
        train_loader = torch.utils.data.DataLoader(dataset, batch_size=args.batch_size,shuffle=True,num_workers=4)

        model = Net2().to(device)
        optimizer = optim.SGD(model.parameters(), lr=args.lr, momentum=args.momentum)

        dataset_test = GigaDataset_LOSO(istrain=False, subj=subj)
