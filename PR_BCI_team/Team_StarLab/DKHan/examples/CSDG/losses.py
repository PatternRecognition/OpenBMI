...
class log_triplet(nn.Module):
    def __init__(self):
        super(log_triplet, self).__init__()
    ...
    def forward(self, anchor, i_out, j_out, gt1, gt2, gt3, size_average=True):
        ...
        A = (torch.log(distance_a_i/gt1))
        B = (torch.log(distance_a_j/gt2))
        #C = (torch.log(distance_i_j/gt3))
        loss_1 = (A-B).pow(2).sum(0)
        #loss_2 = (B-C).pow(2).sum(0)
        loss = loss_1 #loss_2
        return loss.sum()


class log_ratio_loss(nn.Module):
    def __init__(self, margin, gamma):
        super(log_ratio_loss, self).__init__()
        self.margin = margin
        self.gamma = gamma

    def forward(self, anchor, positive, negative, a_label, p_label, n_label, clf_loss, size_average=True):
        distance_a_i = (anchor - positive).pow(2).sum(1)  # .pow(.5)
        distance_a_j = (anchor - negative).pow(2).sum(1)


        A = (torch.log(distance_a_i / gt1))
        B = (torch.log(distance_a_j / gt2))
        loss = (A - B).pow(2).sum(0)
