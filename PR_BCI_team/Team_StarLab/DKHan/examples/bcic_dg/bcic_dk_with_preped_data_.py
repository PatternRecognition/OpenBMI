...
from mynetworks import Deep4Net_origin, ConvClfNet, FcClfNet, EEGNet_v2 , EEGNetCSD
from trte import *
...
def exp(subject_id):
    ...
    crop_size =1125
    model = EEGNetCSD(n_classes, 22, 1125,device)
    ...
    for epochidx in range(1, epochs):
        print(epochidx)
        train_csd(10, model, device, train_loader,optimizer,scheduler,cuda, args.gpuidx)
        val_loss, val_score = eval_csd(model, device, valid_loader)
        test_loss, test_score = eval_csd(model, device, test_loader)
...
if __name__ == '__main__':
    ...
    for id in range(1,10):
        ...
        df_all.to_csv("bcic_remaked_data_[2,50]_eegnet_csd_fixed_p.csv",mode='w')
