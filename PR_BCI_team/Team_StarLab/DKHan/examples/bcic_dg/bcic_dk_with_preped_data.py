...
def exp(subject_id):
    ...
    crop_size =1125
    embedding_net = EEGNet_gabor(n_classes, 22, 1125, device)
    ...
    for epochidx in range(1, epochs):
        ...
        temp = model.embedding_net._modules['convnet'][0].weight
        temp2 = temp.squeeze().cpu().detach().numpy()

        import matplotlib.pyplot as plt
        plt.plot(temp2.transpose())
