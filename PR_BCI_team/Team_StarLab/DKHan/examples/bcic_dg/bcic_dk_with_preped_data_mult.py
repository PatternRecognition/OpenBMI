...
from mynetworks import Deep4Net_origin, ConvClfNet, FcClfNet_mult, EEGNet_v2, Discriminator
from trte import *
...
def exp(subject_id):
    ...
    discriminator = Discriminator([model.embedding_net.num_hidden, 8], grl=True, reverse=True)
    print(model)
    ...
    optimizer_C = optim.AdamW(model.parameters(), lr=0.01, weight_decay=0.5 * 0.001)
    optimizer_D = optim.AdamW(discriminator.parameters(), lr=0.01, weight_decay=0.5 * 0.001)
    ...
    for epochidx in range(1, epochs):
        print(epochidx)
        train_mult(10, model,discriminator, device, train_loader,optimizer_C,optimizer_D,scheduler,cuda, args.gpuidx, epoch=epochidx)
...
if __name__ == '__main__':
    ...
    for id in range(1,10):
        ...
        df_all.to_csv("bcic_remaked_data_mult_fix.csv",mode='w')
