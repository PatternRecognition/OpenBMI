...
def exp(args,fold_idx, train_set,valid_set, test_set):
    ...
    train_loader = torch.utils.data.DataLoader(train_set, batch_size=args.batch_size, shuffle=False, pin_memory=False)
    valid_loader = torch.utils.data.DataLoader(valid_set, batch_size=args.batch_size, shuffle=False,pin_memory=False)
    test_loader = torch.utils.data.DataLoader(test_set, batch_size=args.batch_size, shuffle=False,pin_memory=False)
    ...
    # optimizer = torch.optim.Adam(model.parameters(), lr=args.lr, weight_decay=10e-6)
    ...
    valid_acc = 0
    valid_min_loss = 1000000000000000000000
    best_acc = 0
    best_acc_loss = 0
    max_acc = 0
    ...
    optimizer = torch.optim.AdamW(model.parameters(), lr=1.0, weight_decay=10e-6)
    lambda1 = lambda epoch: epoch*(args.lr/warm_up_step)
    scheduler = torch.optim.lr_scheduler.LambdaLR(optimizer,lambda1)


    for epoch in range(0, warm_up_step):
        start = time.time()
        train(10, model, device, train_loader,optimizer,scheduler,cuda, args.gpuidx, epoch=epoch)
        ...
        print(f'LR : {lr}')
        logger.log_training(train_loss, train_score,valid_loss,valid_score, test_loss, test_score, lr, epoch)
    ...
    optimizer = torch.optim.AdamW(model.parameters(), lr=last_lr, weight_decay=10e-6)
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer,T_max=args.epochs-warm_up_step)
    # scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer,'min',factor=0.1)

    for epochidx in range(warm_up_step, args.epochs):
        print(epochidx)
        start = time.time()

        train(10, model, device, train_loader,optimizer,scheduler,cuda, args.gpuidx, epoch=epochidx)
        print(f'total time: {time.time()-start}')
        utils.blockPrint()
        # train_loss, train_score = eval(model, device, train_loader)
        train_loss =0
        ...
        print(f'LR : {lr}')
        logger.log_training(train_loss, train_score,valid_loss,valid_score, test_loss, test_score, lr, epochidx)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--result-dir', default=f'/{hp.model}_batch{hp.batch_size}_cutout')
    parser.add_argument('--batch-size', type=int, default=hp.batch_size, metavar='N',
                        help='input batch size for training (default: 8)')
    parser.add_argument('--test-batch-size', type=int, default=hp.batch_size, metavar='N',
    ...
    subject_list = np.r_[0:9]
    subject_list = np.r_[9:18]
    os.environ["CUDA_VISIBLE_DEVICES"] = "1"
    subject_list = np.r_[18:27]
    subject_list = np.r_[27:36]
    os.environ["CUDA_VISIBLE_DEVICES"] = "2"
    subject_list = np.r_[36:45]
    subject_list = np.r_[45:54]

    # os.environ["CUDA_VISIBLE_DEVICES"] = "0"
    # subject_list = np.r_[18:21]
    # # os.environ["CUDA_VISIBLE_DEVICES"] = "1"
    # # subject_list = np.r_[21:24]
    # # os.environ["CUDA_VISIBLE_DEVICES"] = "2"
    # # subject_list = np.r_[24:27]
