...
for sess in [1,2]:
    ...
    for sub in range(1,55):
        ...
        cov_data_train_reshape = cov_data_train.reshape(200,400)

        cv = ShuffleSplit(10, random_state=42)
        cv_split = cv.split(cov_data_train_reshape)
        ...
        scores = cross_val_score(clf, cov_data_train_reshape, labels, cv=cv, n_jobs=-1)
