...
class Net2(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Conv2d(1, 100, (62,45), 1)
        #self.conv1_2 = nn.Conv2d(20, 20, (1, 5), 1)
        self.conv2 = nn.Conv2d(25, 25, (62,1), 1)
        self.conv3 = nn.Conv2d(25, 50, (1,10), 1)
        ...
        self.fc1 = nn.Linear(100 * 1 * 83, 500)
        self.fc2 = nn.Linear(100, 10)
...
class GigaDataset_session_wise(Dataset):
    ...
    # Initialize your data, download, etc.
    def __init__(self,transform=None,istrain = True, subj=54, sess=1):
        self.transform = transform
        ...
        if istrain:
            ...
            x_data = x_data[200*subj+100:200*subj+200,:,:,:] #online만 사용
            y_data = y_data[200*subj+100:200*subj+200]
...
def main():
    ...
    parser.add_argument('--batch-size', type=int, default=1060, metavar='N',
    ...
    for subj in range(54,108):
        dataset = GigaDataset_session_wise(istrain = True, subj=subj)
        train_loader = torch.utils.data.DataLoader(dataset, batch_size=args.batch_size,shuffle=True,num_workers=4)

        model = Net2().to(device)
        optimizer = optim.SGD(model.parameters(), lr=args.lr, momentum=args.momentum)

        dataset_test = GigaDataset_session_wise(istrain=False, subj=subj)
        test_loader = torch.utils.data.DataLoader(dataset_test, batch_size=args.batch_size, shuffle=True, num_workers=4)

        for epoch in range(1, args.epochs + 1):
            train(args, model, device, train_loader, optimizer, epoch)
            loss, score = test(args, subj, model, device, test_loader)
            f.write(str(subj)+" "+str(loss)+" "+str(score) + '\n')
