...
class Net2(nn.Module):
    def __init__(self):
        ...
        self.fc1 = nn.Linear(40 * 1 * 83, 500)
        self.fc2 = nn.Linear(100, 10)
        self.fc3 = nn.Linear(10, 100)
...
class GigaDataset_LOSO(Dataset):
    ...
    def __init__(self,transform=None,istrain = True,subj=0):
        ...
        if istrain:
            ...
            x_data = np.delete(x_data, np.s_[200 * (subj+54):200 * (subj+54) + 200], 0)  # 2세션에서 지움
            y_data = np.delete(y_data, np.s_[200 * (subj+54):200 * (subj+54) + 200], 0)
        else:
            #2세션에 온라인만 테스트
...
def main():
    ...
    f = open("base_cnn" + datetime.today().strftime("%m_%d_%H_%M") + ".txt", 'w')
