...
class Attention(nn.Module):
    def __init__(self, attention_size,num_filters,num_hidden):
        ...
        self.attention = new_parameter(attention_size, 1) #43*1
        self.fc_att = nn.Linear(self.num_filters * 1 * 16, self.num_hidden) #640*512

    def forward(self, x_in):
        v = torch.tanh(self.fc_att(x_in)) # xshape -> 512*43 (각 배치에대해)
        #vu = torch.matmul(v,self.attention).squeeze()
        # after this, we have (batch, dim1) with a diff weight per each cell
        self.attention_score = torch.matmul(v, self.attention).squeeze()
        self.attention_score = F.softmax(self.attention_score,1).view(x_in.size(0), x_in.size(1), 1)
...
class Net2(nn.Module):
    def __init__(self):
        ...
        self.attn = Attention(attention_size=43,num_filters=self.num_filters,num_hidden=self.num_hidden)
...
class Net3(nn.Module):
    def __init__(self):
        ...
        self.fc1 = nn.Linear(self.num_filters * 1 * 16, self.num_hidden)
        self.fc2 = nn.Linear(self.num_hidden,self.num_hidden)
        self.fc_fin = nn.Linear(self.num_hidden, 2)

    def forward(self, x):
        x = segment_dataset(x, window_size=200, step=7)
        x = x.view(-1,1,x.size(2),x.size(3)).to("cuda")
        x = self.conv1(x)
        #x = self.bn1(x)
        x = F.elu(x) #temporal

        x = F.max_pool2d(x, (1, 45), 7)
        #x_t = x.view(-1,11,self.num_filters * 1 * 49)
        x = x.view(-1,43,self.num_filters * 1 * 16)
    ...
    for (start, end) in windows(data, window_size, step):
        if(len(data[0,start:end]) == window_size):
            if start == 0:
                ...
                segments = torch.cat([segments, data[:,start:end].unsqueeze(0).unsqueeze(0)],1)
    return segments


def segment_dataset(X, window_size, step):
    win_x = torch.zeros([X.shape[0],43,62,window_size])
    for i in range(X.shape[0]): #각 트라이얼에 대해
        win_x[i,:,:,:] = t = segment_signal_without_transition(X[i].squeeze(), window_size, step)
    return win_x
...
class GigaDataset_session_wise(Dataset):
    ...
    def __init__(self,x,y,transform=None,istrain = True, sess=1,subj=54):
        ...
        #self.x_data = segment_dataset(x_data, window_size=200, step=7)
...
def main():
    ...
    model.load_state_dict(torch.load('04_06_14_5890cam.pt'))
    ...
    if ch == 'y':
        for epoch in range(1, args.epochs + 1):
            ...
            torch.save(model.state_dict(), datetime.today().strftime("%m_%d_%H_%M") + str(epoch) + 'cam.pt')


    loging = True
    if loging:
        fname = "log/190329/cam_0405_2" + ".txt"
        f = open(fname, 'w')
    #subjwisetest

    model = Net3().to(device)
    model.load_state_dict(torch.load('04_05_21_2520cam.pt'))
    for subj in range(54,108):
        ...
        if loging:
            f = open(fname, 'a')
            f.write(str(subj) + " " + "jl : " + str(loss) + " " + str(score) + '\n')
            f.close()
