...
def main():
    ...
    f = open("log/base_cnn_subjwise(trainsess1_testsess2on)2" + datetime.today().strftime("%m_%d_%H_%M") + ".txt", 'w')

    x_data, y_data = load_smt()



    for subj in range(27,54):
    ...
    if (args.save_model):
        torch.save(model.state_dict(), datetime.today().strftime("%m_%d_%H_%M") + 'basecnn.pt')
