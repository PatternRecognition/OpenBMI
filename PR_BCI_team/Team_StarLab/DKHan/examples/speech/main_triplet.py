...
def main(i):
    ...
    if use_split:
        test_idx = np.r_[sum(sizes[0:test_subj]):sum(sizes[0:test_subj + 1])]
        dataset = SpeechDataset(x=x_data, y=y_data, idx=test_idx, train=True)
        ...
        triplet_dataset_train = TripletSpeech(dataset_train,subset=True)
        triplet_dataset_test = TripletSpeech(dataset_test,subset=True)
    else:
        test_idx = np.r_[sum(sizes[0:test_subj]):sum(sizes[0:test_subj + 1])]
        train_idx = np.setdiff1d(np.r_[0:y_subj.shape[0]], test_idx)
