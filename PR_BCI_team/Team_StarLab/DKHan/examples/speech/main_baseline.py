...
def main(i):
    ...
    x_data, y_data, sizes = load_speech(path='')
    x_data = x_data[:,:,:,50:]
    #get subject number
    ...
    if use_split:
        # test_idx = np.r_[sum(sizes[0:test_subj]):sum(sizes[0:test_subj + 1])]
        # dataset = SpeechDataset(x=x_data, y=y_data, idx=test_idx)
        dataset = SpeechDataset(x=x_data, y=y_data, idx=np.r_[0:y_subj.shape[0]])
        ...
        dataset_train = SpeechDataset(x=x_data, y=y_data, idx=train_idx)
        dataset_test = SpeechDataset(x=x_data, y=y_data, idx=test_idx)
