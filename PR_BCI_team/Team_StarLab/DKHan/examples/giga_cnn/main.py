...
def main():
    ...
    if (args.save_model):
        model_save_path = 'model/deep4cnn_2/'
    ...
    if valtype == 'loso':
        ...
        print(model)
        #model.load_state_dict(torch.load(model_save_path+'model_70.pt'))
        ...
        model = ShallowCNN(ismult=ismultitask).to(device)
        optimizer = optim.SGD(model.parameters(), lr=args.lr, momentum=args.momentum)
        nonbciilli = np.s_[0, 1, 2, 4, 5, 8, 16, 17, 18, 20, 21, 27, 28, 29, 30, 32, 35, 36, 38, 42, 43, 44, 51] #sess1 > 0.7
        dataset_train = GigaDataset(x=x_data, y=y_data, istrain=True, sess=1,valtype='subj',subj=nonbciilli)
        train_loader = torch.utils.data.DataLoader(dataset_train, batch_size=args.batch_size, shuffle=True, **kwargs)

        dataset_test = GigaDataset(x=x_data, y=y_data, istrain=False, sess=2, valtype='subj',subj=np.s_[54:108])
        test_loader = torch.utils.data.DataLoader(dataset_test, batch_size=args.batch_size, shuffle=False, **kwargs)
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            j_loss, j_score = eval(args, model, device, test_loader)
            if (args.save_model):
                torch.save(model.state_dict(), model_save_path + "nonillitrain" + str(epoch) + '.pt')
