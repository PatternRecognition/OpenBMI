...
def main():
    ...
    scheduler = lr_scheduler.MultiStepLR(optimizer, milestones=milestones, gamma=0.1)  # 너무 빨리 떨구면 언더피팅하는듯
    ...
    for epochidx in range(100):
        fit(triplet_train_loader, triplet_test_loader, model, loss_fn, optimizer, scheduler, epochidx, n_epochs, cuda, log_interval)
        print(epochidx)
