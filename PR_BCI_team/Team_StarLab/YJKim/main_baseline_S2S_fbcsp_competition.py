...
name_index_ear = ['FT9', 'T7', 'TP7', 'TP9', 'FT10', 'T8', 'TP8', 'TP10']

filter_order = 5
f_range = np.array([[4, 8], [8, 12], [12, 16], [16, 20], [20, 24], [24, 28], [28, 32], [32, 36], [36, 40]])
...
for ii in range(len(subject)):
    ...
    # band-pass filter
    CNT_tr = ob.filter_bank(CNT_tr, f_range, filter_order)
    CNT_te = ob.filter_bank(CNT_te, f_range, filter_order)
    ...
    # feature extraction
    SMT_tr_motor, CSP_W_motor = ob.csp_fb(SMT_tr, n_pattern)

    SMT_tr_motor = ob.project_CSP_fb(SMT_tr, CSP_W_motor)
    SMT_te_motor = ob.project_CSP_fb(SMT_te, CSP_W_motor)

    FT_tr_motor = ob.log_variance(SMT_tr_motor)
    FT_te_motor = ob.log_variance(SMT_te_motor)

    # mutual information based feature selection
    mut_info_motor = ob.mutual_info(FT_tr_motor)
    selected_feat_motor = ob.feat_select(mut_info_motor)

    FT_tr_motor['x'] = FT_tr_motor['x'][selected_feat_motor, :]
    FT_te_motor['x'] = FT_te_motor['x'][selected_feat_motor, :]

    # classification
    sh_LDA_motor = ob.shrinkage_LDA(FT_tr_motor)
    OUT_lda_motor = ob.project_shLDA(FT_te_motor, sh_LDA_motor)

    accuracy[count][2 * ii] = OUT_lda_motor
    count = count + 1
