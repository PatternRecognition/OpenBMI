...
class Figure_Many(Stimulus):             
    ...
    def __init__(self, **kw):
        ...
        self.pos = False
        self.color_on = self.parameters.color_on
        self.color_off = self.parameters.color_off
        # Calculate points
    ...
    def draw(self):
        ...
        if int(p.time_passed * p.frequency_used) % 2: 
            self.colors = self.color_on
            self.colors_font = self.color_off
        else:
            self.colors = self.color_off
            self.colors_font = self.color_on
        ...
        for i in range (p.letters_number):
    ...
    def get_my_letters(self):
        return self.letters_to_use
    
    def set_cols(self, color_on, color_off):
        #print "i was here!!!"
        self.color_on = color_on
        self.color_off = color_off
        
    def set_my_letters(self, use_letters):
        self.letters_to_use = use_letters
    ...
    def calculate_center_of_each_letter(self, number_of_figures, radius_of_figure, distance_between, center):
        ...
        if number_of_figures == 1:
            center = self.parameters.center_position
            f_size = self.parameters.font_size
            return [(center[0] - 0.25 * f_size, center[1] - 0.25 * f_size)]
