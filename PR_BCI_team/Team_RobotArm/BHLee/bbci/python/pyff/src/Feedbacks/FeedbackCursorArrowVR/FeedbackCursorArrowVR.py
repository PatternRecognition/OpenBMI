...
# For the tcp connection with the Virtual Environment
import socket

from FeedbackBase.MainloopFeedback import MainloopFeedback


class FeedbackCursorArrowVR(MainloopFeedback):
...
# New markers, VR experiment
# "class requested + class selected"
# 11: hit class 1 (left)       											HIT
# 22: hit class 2 (right)      											HIT
# 12: wrong class selected; requested 1, selected 2  ERROR
# 21: wrong class selected; requested 2, selected 1  ERROR
# 10: no option selected when 1 was requested        REJECT
# 20: no option selected when 2 was requested        REJECT
    ...
    def on_init(self):
        ...
        self.introtext = ["Get ready to begin the BCI experiment.", "Have fun"]
        ...
        self.durationActivationPeriod = 20000
        
        self.hitMissDuration =  1000
        self.pauseDuration = 10000
        self.introDuration = 10000        

        self.countdownFrom = 10
        self.shortPauseCountdownFrom = 10
        ...
        #VR: conversion from the "cross" world to the VR units
        self.posVR = 0
        self.posFB = 0
        self.FBscale = 0.8    
        # The parameters concerning the selection time and threshold of each class are loaded by the call to the "set" function
        self.pauseBeforeFB = 1000

        self.feedbackWithVR = True
    ...
    def init_tcp_connection(self):
        ...
        # VR              
        # The Virtual Environment is supposed to be already running, listening in the port "40000"
        # Let's connect with it
        # create a socket
        self.vrSocket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)

        # connect to server
        host = 'localhost'
        port = 40000
        self.vrSocket.connect((host, port))    
    ...
    def generate_trials3(self):
       ...
       # same number of class-trials in segments between pauses
       nPauses = self.totalTrials / self.pauseAfter;
       nTrialsPerSegment = self.totalTrials % self.pauseAfter;
       end = False;

       nT = 0

       for p in xrange(len(self.classTrialsPerPause)):

           nTrialsPerClass = self.classTrialsPerPause[p] / len(self.classes)
           auxTrialDirections = [0] * nTrialsPerClass *  len(self.classes)

           for c in xrange(len(self.classes)):
               auxTrialDirections[int(nTrialsPerClass)*c:int(nTrialsPerClass)*(c+1)] = [c+1]* int(nTrialsPerClass)

           random.shuffle(auxTrialDirections)

           self.trialDirections[nT:nT+len(auxTrialDirections)] = auxTrialDirections
           nT = nT + len(auxTrialDirections)
    ...
    def generate_trials2(self):
        ...
        self.totalTrials = len(self.trialDirections)
        print("tralara trials = %d" % self.totalTrials); 	  
    ...
    def pre_mainloop(self):
        ...
        #VR
        self.screenPosX =  self.screenPos[0]
        self.screenPosY = self.screenPos[1]
        self.screenWidth =  self.screenPos[2]
        self.screenHeight = self.screenPos[3]
        #/VR  


        if(self.feedbackWithVR):           
    ...
    def play_tick(self):
        ...
        if self.intro:
            ...
            self.pause_tick()
        elif self.hit or self.error or self.reject:
            self.hit_miss_tick()
        elif self.gameover:
    ...
    def feedback_trial_tick(self):
        ...
        if self.firstTickOfTrial:
            ...
            #VR: during (part of) the pause before the fixation cross appears, the arm
            #should move back to the original position
            # For example; let's take the half of the pause         
            self.tickMov = ( self.posVR / (self.pauseBeforeFixationCross/2.0) )
            self.pos_tmp = self.posVR
            #/VR
            ...
            self.timeOverRight = 0
            self.timeOverFeet = 0
            
        # Teste ob zeit fuer alten Trial abgelaufen ist
        if self.trialElapsed >= self.durationTrial:            
            #self.check_for_hit_miss(); 
            self.check_for_hit_miss_VR();
            return
      
        if self.trialElapsed < self.pauseBeforeFixationCross:
            self.draw_all(False, False)
            
            if self.trialElapsed < self.pauseBeforeFixationCross/2:
                #VR: during (part of) the pause before the fixation cross appears, the arm
                #should move back to the original position
                self.posVR = ( self.pos_tmp - (self.trialElapsed * self.tickMov) )
                myString = "arm %f" % self.posVR  
                myString = myString.replace('.', ',')
                if(self.feedbackWithVR):                       
                    self.vrSocket.send(myString)
        ...
        elif self.trialElapsed < self.pauseBeforeFixationCross + self.durationFixationCross + self.pauseBeforeFB:   
            # Just wait, we do not provide FB during this time
            if self.firstTickOfArrow:
                print("class %s" % self.classes[self.targetDirection])
                self.firstTickOfArrow = False
                self.send_parallel(self.markers[self.targetDirection])
            ...
            #VR feedback                                                                                         	
            self.posFB = self.pos * self.FBscale * self.screenWidth/2.

            if(self.feedbackWithVR):               
                self.posVR = self.pos
                #print('posVR = %.2f' % self.posVR)
                # Here we should send through a tcp socket the value "self.posVR" to the virtual environment
                myString = "arm %f" % self.posVR     
                myString = myString.replace('.', ',')                                   
                self.vrSocket.send(myString)
            
            # We check here if the feedback bar is above the threshold                  
            if ((self.pos) >= (self.threshold2)):
                self.timeOver2 += self.elapsed
                self.timeOver1 = 0
                if (self.timeOver2 >= self.selectTime2):
                    print('SELECTION: RIGHT')
                    self.check_for_hit_miss_VR()

            elif ((self.pos) <= (self.threshold1)):
                self.timeOver1 += self.elapsed
                self.timeOver2 = 0
                if (self.timeOver1 >= self.selectTime1):
                    print('SELECTION: LEFT')
                    self.check_for_hit_miss_VR()
    ...
    def intro_tick(self):
        ...
        self.draw_init()
        self.do_print(self.introtext, self.countdownColor, self.size/6, True)
    ...
    def short_pause_tick(self):
        ...
        self.do_print(str(t), self.countdownColor, None, True)
    ...
    def end_of_experiment_tick(self):
        ...
        if self.calibration:
            ...
            myStr = "[Hits %d, Errors %d, Rejections %d]" % (self.hitErrorReject[0], self.hitErrorReject[1], self.hitErrorReject[2])
            self.do_print(myStr, None, self.size/3, False)
            print('GAME OVER')
            print('[Hits, Errors, Rejections]:')
            print(self.hitErrorReject)
            self.send_parallel(self.GAME_OVER)
    ...
    def hit_miss_tick(self):
        ...
        if self.hitMissElapsed >= self.hitMissDuration:
            ...
            self.hit, self.miss, self.reject, self.error = False, False, False, False            
        ...
        if(self.calibration == False):
            ...
            if self.hit:
                s = "Hit"
                self.hitErrorReject[0] += 1
            elif self.error:
                s = "Error"
                self.hitErrorReject[1] += 1
            elif self.reject:
                self.hitErrorReject[2] += 1
                s = "Reject"
    ...
    def check_for_hit_miss_VR(self):                     
        if self.trialElapsed < self.durationTrial:                  
            if (self.pos<0 and self.targetDirection == 0):
                self.hit = True
                self.send_parallel(self.VR_HIT_1)
                print('HIT LEFT')
            elif (self.pos>0 and self.targetDirection == 1):
                self.hit = True
                self.send_parallel(self.VR_HIT_2)
                print('HIT RIGHT')
            elif (self.pos<0 and self.targetDirection == 1):
                self.error = True
                self.send_parallel(self.VR_ERROR_2)
                print('ERROR RIGHT')
            elif (self.pos>0 and self.targetDirection == 0):
                self.error = True
                self.send_parallel(self.VR_ERROR_1)
                print('ERROR LEFT')
                            
        else:
            self.reject = True
            if self.targetDirection == 0:
                self.send_parallel(self.VR_MISS_1)
                print('REJECT LEFT')
            else:
                self.send_parallel(self.VR_MISS_2)
                print('REJECT RIGHT')
    ...
    def update_cursor(self):
        ...
        #self.posFB = min(border, max(-border, self.posFB));
        arrowPos = { self.LEFT  : (-abs(self.posFB),0),
                     self.RIGHT : (abs(self.posFB),0),
                     self.DOWN  : (0,abs(self.posFB)),
                     self.UP    : (0,-abs(self.posFB))
                   }[self.direction]
    ...
    def init_graphics(self):
        ...
        self.arrow.fill(self.borderColor)
        
        #pygame.draw.polygon(self.arrow, self.arrowColor, scaledArrow)
        pygame.draw.polygon(self.arrow, self.backgroundColor, scaledArrow)
        ...
        pygame.draw.line(self.cursor, self.cursorColor, (0,scale/2),(scale,scale/2), scale/10)        
        pygame.draw.line(self.cursor, self.cursorColor, (scale/2,0),(scale/2,scale), scale/10)
        ...
        self.border = pygame.Surface((self.screen.get_width(), self.screen.get_height()))
        #self.border = pygame.Surface((self.screen.get_width() * self.FBscale, self.screen.get_height()))
        ...
        self.inner = pygame.Surface((self.size, self.size))
        #self.inner.fill(self.backgroundColor) 
        self.inner.fill(self.borderColor)    
        ...
        if not self.calibration:
            ...
            self.punchline1Rect = self.update_punchline(self.punchline1, self.sign[self.classes[0]] * self.screenWidth/2. * self.FBscale * self.threshold1, self.classes[0])
            ...
            self.punchline2Rect = self.update_punchline(self.punchline2, self.sign[self.classes[1]] * self.screenWidth/2. * self.FBscale * self.threshold2, self.classes[1])
