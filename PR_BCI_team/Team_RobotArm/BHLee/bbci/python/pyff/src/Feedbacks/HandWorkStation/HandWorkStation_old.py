...
# HandWorkStation.py - HandWorkStation Game
...
class HandWorkStation(PygameFeedback):
    ...
    TRIG_START_NORMAL = 41
    TRIG_START_STRESS = 42
    ...
    def pre_mainloop(self):
        ...
        if self.MODE == 1:
            # 1: PRACTICE/TESTING
            self.NR_BLOCKS = 1
            self.BLOCK_DURATION = 200.0 # seconds
            self.BLOCK_SPEED = [.8]
            self.BLOCK_STRESS = [False]
            self.DIST_STRESS_H = [.3]
            self.DIST_STRESS_L = [.45]
            self.save_log = False
        elif self.MODE == 2:
            # 2: SPEED CALIBRATION
            self.NR_BLOCKS = 4
            self.BLOCK_DURATION = 100.0 # seconds
            self.BLOCK_SPEED = linspace(.8, .99, self.NR_BLOCKS)
            self.BLOCK_STRESS = [False]*self.NR_BLOCKS
            self.save_log = True
            self.log_fname = self.DATA_DIR + 'Calibration_1__' + time.strftime("%m-%d-%Y--%H-%M-%S", time.gmtime()) + '.txt'
        elif self.MODE == 3:
            # 3: DISTANCE CALIBRATION
            self.NR_BLOCKS = 4
            self.BLOCK_DURATION = 100.0 # seconds
            self.BLOCK_SPEED = [self.BLOCK_SPEED]*self.NR_BLOCKS
            self.BLOCK_STRESS = [True]*self.NR_BLOCKS
            self.DIST_STRESS_H = [0.45, 0.4, 0.35, 0.3]
            self.DIST_STRESS_L = [.495, .475, .45, .425]
            self.save_log = True
            self.log_fname = self.DATA_DIR + 'Calibration_2__' + time.strftime("%m-%d-%Y--%H-%M-%S", time.gmtime()) + '.txt'
        elif self.MODE == 4:
            # 4: EXPERIMENT
            self.NR_BLOCKS = 16
            self.BLOCK_DURATION = 90.0 # seconds
            self.BLOCK_SPEED = [self.BLOCK_SPEED]*self.NR_BLOCKS
            self.BLOCK_STRESS = [False, True]*int(self.NR_BLOCKS/2)
            self.DIST_STRESS_H = [self.DIST_STRESS_H]*self.NR_BLOCKS
            self.DIST_STRESS_L = [self.DIST_STRESS_L]*self.NR_BLOCKS
            self.save_log = False
        ...
        # initialize counters
        self.tick_count = -self.FPS*3   # initial tick count
        self.missed = 0
        self.correct = 0
        self.wrong = 0
        self.block_nr = 1
    ...
    def play_tick(self):
        ...
        if self.timer.elapsed > self.BLOCK_DURATION:
