...
from VEShapes import HexagonOpening, StripeField
from CakeSpellerVE import CakeSpellerVE

class CenterCakeSpellerMVEP(CakeSpellerVE):
    """
    Visual speller as a cake with six pieces with a moving field in
    the center.
    ...
    def init(self):
        ...
        self.stripes_speed      = 0.03
        self.stripes_angle      = .3       # angle of arrows (0.5 -> 90 degrees)
        self.stripes_count      = 12       # number of stripes
        self.stripes_distance   = 20.0     # distance between stripes
        self.stripes_line_width = 1.5      # thickness of stripes
    ...
    def init_screen_elements(self):
        ...
        # Stripe field stimulus
        self._ve_stripes = \
            StripeField(on=False,
                        num_stripes=self.stripes_count,
                        line_width=self.stripes_line_width,
                        center=(self._centerPos[0], self._centerPos[1]),
                        width=self.stim_radius * 2., angle=self.stripes_angle,
                        dist_stripes=self.stripes_distance,
                        anti_aliasing=True)
        a = self.speller_radius * self.stripes_speed
        self._stripes_end_pos = [(
        ...
        self._ve_elements.insert(0, self._ve_stripes)
