#!/usr/bin/env python
#
# Loads images and presents them in perspective views. Needs VisionEgg installed.
#
# Copyright (C) 2009  Helene Schmidt, Matthias Treder
#
...
class Perspective(MainloopFeedback):
    ...
    def init(self):
        ...
        self.ranSym = (0,1)
        self.angles = (-20,-10,-5,0,5,10,20)        # Slant angles
        ...
        # Stimuli
        self.basedir = 'D:\stimuli\gert' # Path to directory with stimuli
        self.subdirs = ('ran','sym')
        self.postfix = '.jpg'
        ...
        # Screen Settings
        self.screenPos = [100, 100, 1000, 700]
        self.bgColor = (0., 0., 0.)
    ...
    def __init_screen(self):
        ...
        # make stimuli:  
        self.ve_pic = TextureStimulus3D(on=False,
                                        shrink_texture_ok=True,)
        
        # make mask
        #self.mask = Mask2D(function='circle',radius_parameter= 250.)
        #self.ve_mask = TextureStimulus(anchor='center',
        #                               position = (screenWidth/2., screenHeight/2.),
        #                               mask = self.mask,
        #                               max_alpha = 0.0,
        #                               on = False)
    ...
    def trial(self):
        ...
        self.pic = os.path.join(self.basedir,self.subdirs[ranSym],str(pic_nr)+self.postfix)
        self.texture = Texture(self.pic)
        pic_length,pic_width = self.texture.size
        self.ve_pic.set(texture=self.texture,                                        
                        lowerleft =  (-pic_length/2., -pic_width/2.,0.),
                        lowerright = (pic_length/2., -pic_width/2.,0.), 
                        upperleft =  (-pic_length/2., pic_width/2.,0.),
                        upperright = (pic_length/2., pic_width/2.,0.)   )

        # projection
        self.projection = SimplePerspectiveProjection()
        self.projection.look_at((0.,0.,2000.),(0.,0.,0.),(0.,1.,0.) ) # look_at(self, eye, center, up)
        self.projection.rotate(angle,0.,1.,0.)
        self.viewport_pic.set(projection = self.projection)
