...
#            [13.1.11, 00.30]
#       5. store distractor trial no's as well [21.1.11, 08.39]
#       6. empty backspace prints backspace!!
#       7. distractor sequence not entering properly. Fixed [31.1.11, 10.30]
#       8. problem with error, stops everything somehow save python workspace
#            and have option to start from where it stopped
#       9. parallel writing of store_ files (imp)
#       10. display correct percentage longer. FIXED [31.1.11, 21.49]
#       11. triggers for first key press and final answer!! FIXED [31.1.11, 21.50]
#       12. First key press of the final word might be more useful and the first
#            key press on the keyboard too [31.1.2011, 21.46]
#NOTE:  1. os.environ added to pygame_init, so the pygame window fits exactly
#           to the secondary monitor. For laptop, this has to be changed.When fullscreen is off.
#OPTION: 1. refactoring: for ex. use same scheme of indexing for all to extract
#            the words. distractor seems to have a different sequence currently
#            [21.1.11, 6.38]
#        2. convert all the parts of the code used more than once into functions
...
class VocabularyDeveloperFeedback(MainloopFeedback):
    ...
    def init(self):
        ...
        print "+++++++++++++++++++++++++++++++++++++++++++++++"
        print "HOORAY, VocabularyFeedbackDeveloper HAS BEEN INITIALIZED FROM PYFF"
        print "***********************************************"
...
##                                     +str(localtime[5]) \
##                                     +'.txt'
        self.logfilename = 'logfile_'+self.VPlog+'_' \
                                     +typSes \
                                     +str(self.part) \
                                     +'.txt'
        ...
        if ((self.part == 7) and not(self.finnish)):
            self.part_final = self.part
            self.part = 3
            self.Filenames = ['lektion_simple_1.txt', 'lektion_simple_2.txt','lektion_simple_3.txt','lektion_simple_4.txt']
    ...
    def play_tick(self):
        ...
        # here all words are asked at the very end
            ...
            for filename in self.Filenames:
                ...
                final_indices.extend(self.dict_indices)
...
#            self.dictionary = self.dictionary[0:self.max_dict_length]   
    ...
    def update_bag(self, pair_to_remove):
        ...
        self.learnt_words += 1
        print '+++++++++++++++++++++++++++++++++++++++++++++++++'
        print 'Learnt Words: '+str(self.learnt_words)+'/'+str(len(self.dict_indices))
        print '*************************************************'
...
#            print 'self.bag= '+str(self.bag)
            if len(self.showed_sequence) < self.show_display_limit+1:
                ...
                print 'show_seq= '+str(self.showed_sequence[-1*self.show_display_limit:])
...
#            self.do_print(['The answer has not been correct:', '', pair[0], 'means', pair[2]], size=30)
    ...
    def ask_answer(self, pair, index, give_feedback = True):
            ...
            for event in pygame.event.get():
                if (event.type == pygame.KEYUP):
                    if wait_for_1st_press:
                        time3 = time.time()
                        self.send_parallel(self.FIRST_PRESS)
        ...
        if give_feedback:
            if correct:
                self.do_print(':) CHEERS! Your answer was correct :)', color=(50,155,50),\
                                                                        size=30)
                time.sleep(self.user_feedback_time)
            else:
                self.do_print([':( SORRY! The answer was not correct :( :', \
                            '', " - ".join(pair), "Press enter to continue"], \
                            color=(215,50,50), \
                            size_list=[30, 30, None, 20], center=2)
                self.wait_until_enter2()
...
#       so that the pygame window opens and fits exactly on the display screen 
        if self.hostname == 'Rithwik-PC':
            ...
            os.environ['SDL_VIDEO_WINDOW_POS'] = str(-1920)+","+str(-152)
        if self.fullscreen:
