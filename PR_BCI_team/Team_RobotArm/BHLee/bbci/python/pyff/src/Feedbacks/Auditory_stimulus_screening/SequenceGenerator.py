...
class SequenceGenerator:       
    ...
    def __init__(self, minDistance=3, nstim=9,  minnumMarkerSequences=3,\
        ...
        self.nRandomSeqBefore = random.random_integers(1, maxNonmarkerSeq)
        ...
        self.seq = self.computeRandomSeq(self.numMarkerSequences + self.nRandomSeqBefore)
    ...
    def block(self, last):
        ...
        # block the given last stimuli
        blocked = last 
    
        # block row; same pitch
        if 1:
            if stim in [1,2,3]: blocked += [1,2,3]
            elif stim in [4,5,6]: blocked += [4,5,6]
            elif stim in [7,8,9]: blocked += [7,8,9]

        # block column; same speaker
        if 1:
            if stim in [1,4,7]: blocked += [1,4,7]
            elif stim in [2,5,8]: blocked += [2,5,8]
            elif stim in [3,6,9]: blocked += [3,6,9]
    ...
    def computeRandomSeq(self, numseq):
        ...
        while nseq < numseq:
            subseq = range(1,self.nstim + 1)
            tmpseq = []

            while subseq:
                blocked = self.block((seq+tmpseq)[-self.minDistance:]) # block fields depending on last stimuli
                ...
                s = rchoice(unblocked) # rchoice = random.choice; name change due to random being numpy.random
                tmpseq.append(s)
                subseq.remove(s)
...
if __name__ == "__main__":
    ...
    for zz in range(1000):
        s = ss.computeRandomSeq(50)
        cc=range(1,10)
        for ii in range(1,10):
            cc[ii-1] = s.count(ii)    
        if len(set(cc)) > 1:
            print 'ERROR with unbalanced stim'
        
        for ii in range(3,len(s)+1):
            sub = s[(ii-3):ii]        
            #print sub
            if len(set(sub)) < 3:
        ...
        for ii in range(1,len(s)):
            if s[ii] in blockedList[s[ii-1]]:
                print 'ERROR with 2 neighbors'
                    
    
    print 'Konrad did a good job!'
