...
class CenterOddballBase(MainloopFeedback):
    '''
...
#    ERROR_POTENTIAL = 96 # send if error potential is classified
    ...
    def init(self):
        ...
        '''
        self.log_filename = 'CenterOddballBase.log'
...
#        self.screenPos = [0, 1200, 1280, 730]
        ...
        self.screenPos = [-1919, 0, 1920, 1200]  # the lab screen
        self.screenPos = [1921, 0, 1920, 1200]  # the lab screen
        
           
        self.paused = False
        self.auditory_stimulation = False
        self.visual_stimulation = True
        self.VA_simutaneous = True
        self.audi_visual_depend = False
        self.stimulus_duration_A_Before = 0.150
        
        self.stimulus_duration = 0.130   # 5 frames @60 Hz = 83ms flash
        self.interstimulus_duration = 0.170
        self.nr_sequences = 12
        ...
        self._centerPos = (self.screenPos[0]+(self.screenPos[2])/2., self.screenPos[1] + self._spellerHeight/2.)
        self._centerPos = ((self.screenPos[2])/2., self._spellerHeight/2.)
        print    self._centerPos
...
#        ##  1 ########################initialize the screen parameters
        ...
        self.tm_e=0
        self.osc_size = 40
        self.font_size_phrase = 80       # the spelled phrase at the top
        self.font_size_current_letter = 100       # the spelled phrase at the top
        self.font_size_countdown = 250  # number during countdown
        self.desired_phrase = "3412"
        self.desired_cond = "1234"
        self.cond = 2
        self.auditory_cue_radius = 500   # the cirle for auditory countdown
        ## colors:
        self.bg_color = (0., 0., 0.)
        ...
        #self.Oddshape_set=[['1'],['2'],['3'],['4'],['5'],['6']]
        self.Oddshape_set="123456"
#    
...
#        self.nCountdown = 3            
        ...
        self.shapes = [
            ...
            [ "hourglass", {"size": 150 }], 
            [ "cross", {"orientation": 45, "size": [90,200], "innerColor":self.bg_color}], 
            [ "triangle", {"innerColor":self.bg_color, "innerSize": 60, "orientation": 180, "size": 200}], 
            [ "hourglass", {"orientation": 90, "size": 100}], 
            [ "cross", {"size": [90,200], "innerColor":self.bg_color }],
            ["arrow",{"size":[200,30]}],
            [ "hourglass", {"orientation": 135, "size": 80}]
        ]
        
        self.shapes = [
            [ "triangle", {"innerColor":self.bg_color, "innerSize": 100, "size": 300}], 
            [ "hourglass", {"size": 150 }], 
            [ "cross", {"orientation": 45, "size": [50,270], "innerColor":self.bg_color}], 
            [ "triangle", {"innerColor":self.bg_color, "innerSize": 100, "orientation": 180, "size": 300}], 
            [ "hourglass", {"orientation": 90, "size": 150}], 
            [ "cross", {"size": [50,270], "innerColor":self.bg_color }],
            ["arrow",{"size":[300,80]}],
            [ "hourglass", {"orientation": 135, "size": 80}]
        ]
#
...
#            assert s in [l for ls in self.Oddshape_set for l in ls] # invalid letters in desired phrase!
...
#        self._offline_current_letter = None
    ...
    def __init_screen(self):
        ...
        self._ve_elements.extend([self._ve_countdown, self._ve_oscillator,self._auditory_cue])
...
#            self.post__trial()
    ...
    def __countdown_0(self):
#        for i in xrange(self._nr_elements*self._nr_elements):
#            self._ve_letters[i].set(color = self.letter_color,on= False)
        ...
        self._ve_countdown.set(text="%d" % (self._nr_elements_A-self._current_countdown+1))
        #
...
#            self._ve_letters[i*(self._nr_elements)+self._nr_elements_A-self._current_countdown].set(color = self.letter_color)
        ...
        if self._current_countdown == 0:
            if self.offline:
                for e in xrange(6):
                    if self._desired_letters[:1] in self.Oddshape_set[e]:
                        self.tm_e = e
                self._ve_countdown.set(on=False, text=" ")
                for i in xrange(3):                    
                    self.start_auditory_cue(self.tm_e % self._nr_elements)
                    self._auditory_cue.set(position = self.auditory_cue_pos[self.tm_e],on = True)
#                    else:
    ...
    def __countdown(self):
        ...
        if self._current_countdown == self.nCountdown:
            ...
            if self.offline:
                ...
                self.send_parallel(10*int(self.cond))                
                self.logger.info("[TRIGGER] %d" % (10*int(self.cond)))
                
                
                for i in xrange(3): 
                    self._presentation.set(go_duration=(self.stimulus_duration, 'seconds'))
                    if self.auditory_stimulation:
                        self.start_auditory_cue(self.tm_e % self._nr_elements)                  
                    self._auditory_cue.set(position = self.auditory_cue_pos[self.tm_e],on = True)
                    self._presentation.go()
                    if self.visual_stimulation:
                        self._auditory_cue.set(position = self.auditory_cue_pos[self.tm_e],on = False)
                    self._presentation.set(go_duration=(self.interstimulus_duration+1, 'seconds'))
                    self._presentation.go()
            self._auditory_cue.set(on = False)
        for x in xrange(2):
...
#            return
        ...
        if len(self._desired_letters) > 0:
            ...
            if self._desired_letters[:1] == self.Oddshape_set[currentStimulus]:
        ...
        if self.auditory_stimulation:
            ...
            self.send_parallel(self.STIMULUS[self._current_level-1][currentStimulus] + auditory_marker)
            self.logger.info("[TRIGGER] %d" % (self.STIMULUS[self._current_level-1][currentStimulus] + auditory_marker))
        
        if (not self.VA_simutaneous) and self.visual_stimulation and self.auditory_stimulation:
            self._presentation.set(go_duration=(self.stimulus_duration_A_Before, 'seconds'))
            self._presentation.go()
        ...
        if (not self.VA_simutaneous)and self.visual_stimulation and self.visual_stimulation:
            self._presentation.set(go_duration=(self.interstimulus_duration-0.15, 'seconds'))
        else:
            self._presentation.set(go_duration=(self.interstimulus_duration, 'seconds'))
        self._presentation.go()
...
#                self._ve_letters[self._nr_letters + i].set(on=False)
    ...
    def __classify(self):
        ...
        if self.offline:
            self._classified_element = self.tm_e
...
#        spelled_letter = self.Oddshape_set[self._classified_element]
...
#                self.logger.info("[TRIGGER] %d" % (self.ERROR_POTENTIAL))                    
        ...
        ## update phrases:
        if ( self.copy_spelling and # if copyspelling off, we have no backdoor selected
...
#                    winsound.PlaySound(self.soundfile, winsound.SND_ASYNC)
    ...
    def pre_play_tick(self):
        ...
        if self.cond == "1":
            ...
            self.visual_stimulation = True
            self.auditory_stimulation = True
            self.audi_visual_depend = True
            self.VA_simutaneous = False
        elif self.cond == "4":
            self.visual_stimulation = True
            self.auditory_stimulation = True
            self.audi_visual_depend = True
...
if __name__ == '__main__':
    import Center_Oddball
    logging.basicConfig(level=logging.DEBUG)
    fb = Center_Oddball.Center_Oddball()
    fb.on_init()
    fb.on_play()
