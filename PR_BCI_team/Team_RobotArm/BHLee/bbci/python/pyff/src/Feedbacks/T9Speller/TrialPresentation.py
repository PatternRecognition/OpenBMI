...
class TrialPresentation: 
    ...
    def __init__(self, screenPos, sounds, templateTexts, buttonLabels):
    ...
    def init_pygame(self, screenPos):
        ...
        #self.screenPos = [1000, 10, 1024, 768]
        #self.screenPos = [-1920, 0, 1920, 1200]; #for the lab computer!
    ...
    def init_graphics(self):
        ...
        self.stdAlphabetLegend = ["DUMP","MENÜ","abc","def","ghi","jkl","mno","pqrs","tuv","wxyz"]
...
#            self.scrolledList.items.append(item)
    ...
    def draw_initial(self):
        ...
        for i in arange(1,10):
            ...
            self.do_print(self.buttonLabels[i], self.stdColor, self.fontSizeBig, tuple(array(self.headerPos[i]) - [0,0]), True)
    ...
    def changeButton(self, i, color):
        """
        set buotton (i.e. visual representation of one class) to a specified colour
        """
        button = self.buttonList[i]
        buttonRect = self.buttenRectList[i]
        pygame.draw.rect(button, color, self.rectSizes, 3)
        self.screen.blit(button, buttonRect)
        self.do_print(self.buttonLabels[i], color, self.fontSizeBig, tuple(array(self.headerPos[i]) - [0,0]), True)
...
#    
    sounds = ["sounds/BUK_set8/1.wav", "sounds/BUK_set8/2.wav", "sounds/BUK_set8/3.wav", \
          "sounds/BUK_set8/4.wav", "sounds/BUK_set8/5.wav", "sounds/BUK_set8/6.wav", \
          "sounds/BUK_set8/7.wav", "sounds/BUK_set8/8.wav", "sounds/BUK_set8/9.wav"]
    
    templateTexts = ["ja", "nein", "Alarm", "umlagern", "Schmerzen", "Hallo", "Danke", "Pause"]
    buttonLabels = ["DUMMY", "ti", "it", "ti", "ta", "at", "ta", "to", "ot", "to" ]

    screenPos = [100, 100, 800, 600]
    tp = TrialPresentation(screenPos, sounds, templateTexts, buttonLabels)
    ...
    while 1:
        ...
        if len(randSeq) > 1:
            ...
            #randSeq = range(10)[1:]
            if inMode2: tp.show_mode1()
            else: tp.show_mode2(['nehmen', 'meinen', 'meinem', 'Nehmen'])
            inMode2 = not inMode2
