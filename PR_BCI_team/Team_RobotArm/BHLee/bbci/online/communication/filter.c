...
static void filterFIRCreate(double* filter, int size, int nChans) {
  ...
  reSampleFilter = malloc(reSampleFilterSize * sizeof(double));
  memcpy(reSampleFilter,filter,reSampleFilterSize * sizeof(double));
  
  reSampleFilterValues = malloc(nChans * sizeof(double));
  for(i = 0; i < nChans;++i) {
...
static void filterIIRCreate(double* aFilterPtr, double* bFilterPtr,int fSize, int nChans) {
  ...
  bFilter = malloc(filterSize * sizeof(double));
  aFilter = malloc(filterSize * sizeof(double));
  ...
  zBuffer = malloc(filterSize * channelCount * sizeof(double));
