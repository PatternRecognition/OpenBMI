function [flag,timeshift] = getCondition(condition, condition_param, ...
						    cl, packetLength)
%function [flag,timeshift] = getCondition(condition, condition_param, ...
%						    cl, packetLength)
%
...
if isempty(condition)|isnan(condition)
  ...
  [flag,timeshift,condition] = parseCond(condition,...
					 condition_param,cl, ...
					 packetLength);
...
function [flag,timeshift,condition] = parseCond(condition, ...
						condition_param,cl, ...
						packetLength)
...
switch(condition(1))
 ...
 case 'M'
  % markerCond
  [flag,timeshift,condition] = markerCond(condition, ...
					  condition_param,cl,packetLength);
 case 'F'
...
function [flag,timeshift,condition] = markerCond(condition, ...
						 condition_param, ...
						 cl,packetLength)
...
% wanted markers.
if strcmp(mrkFound,'actiCAP Data On'),
    mrkFound= '';
end
i = [];
...
if ~isempty(i) 
  % we found a matching marker. calculate the timeshift!
   ...
   if mrkArray{2}(1)>=0
    % information only before the beginning of the timeshift should be taken into
    % account.
