...
if ~isempty(end_of_adaptation_marker) & ...
  ...
  %    cls(1).C.invcov= adimexp(curr_feat, UC, cls(1).C.invcov, 0);
      currfeat=[1; curr_feat];
      v= cls(1).C.extinvcov*currfeat;
      cls(1).C.extinvcov= (1/(1-UC))*(cls(1).C.extinvcov-UC/(1-UC+UC*currfeat'*v)*v*v');
      cls(1).C.extinvcov=0.5*(cls(1).C.extinvcov+cls(1).C.extinvcov');
