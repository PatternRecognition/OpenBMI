function [cls,bbci]= bbci_adaptation_pmean(cls, bbci, ts)
...
if ts-ts_trialstart >= bbci.adaptation.adaptation_ival(1) & ts-ts_trialstart <= bbci.adaptation.adaptation_ival(2),
  %fprintf('     %d - %d\n', ts-ts_trialstart, window_counter);
  fn = cls(1).fv;
  new_feat = getFeature('apply',fn,0);
  %curr_feat= curr_feat + new_feat.x;
  curr_feat(:,window_counter+1) = new_feat.x;
...
if ~isempty(end_of_adaptation_marker)
  ...
  if window_counter>=50,
    % adapt bias by PMean method
%    curr_feat= curr_feat/window_counter;
    feat = mean(curr_feat(:,size(curr_feat,2)-15:size(curr_feat,2)),2);
    cls(1).pmean= (1-bbci.adaptation.UC)*cls(1).pmean + ...
        bbci.adaptation.UC*feat;
    cls(1).C.b= -cls(1).C.w' * cls(1).pmean;
