...
for ii= 1:nPat,
  ...
  plot(ci1, movingAverage(X(ii,ci1)', 10, 'method','centered'), ...
       'Color',max(0.6, opt_grid.colorOrder(class_combination(ci,1),:)), ...
       'LineWidth',2);
  hold on;
  plot(ci2, movingAverage(X(ii,ci2)', 10, 'method','centered'), ...
       'Color',max(0.6, opt_grid.colorOrder(class_combination(ci,2),:)), ...
       'LineWidth',2);
  plot(ci1, X(ii,ci1), '.', ...
       'Color',opt_grid.colorOrder(class_combination(ci,1),:));
  plot(ci2, X(ii,ci2), '.', ...
       'Color',opt_grid.colorOrder(class_combination(ci,2),:));
  hold off;
  set(gca, 'XLim', [-1 size(X,2)+2]);
  title(sprintf('{\\bf %s}: %.2f', features.clab{ii}, Xfs(ii)));
end
handlefigures('next_fig', 'Features');
drawnow;