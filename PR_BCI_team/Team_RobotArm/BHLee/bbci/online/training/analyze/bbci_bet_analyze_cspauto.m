...
if bbci.withgraphics 
  ...
  if diff(opt.ival)>=opt.min_ival_length,
    ...
    if strcmpi(opt.enlarge_ival_append, 'start')
      spec= cntToEpo(Cnt, this_mrk, opt.ival(2)+[-opt.min_ival_length 0], 'clab',requ_clab);
    elseif strcmpi(opt.enlarge_ival_append, 'end')
      spec= cntToEpo(Cnt, this_mrk, opt.ival(1)+[0 opt.min_ival_length], 'clab',requ_clab);
    else
...
if ischar(opt.usedPat) & strcmpi(opt.usedPat,'auto'),
  [fv2, hlp_w, la, A]= proc_csp_auto(fv, 'patterns',6);
else
