...
[fv, sel_w]= proc_selectChannels(fv, opt.clab_csp);
...
%% BB: this validation takes the selection of laplacian filters
...
if bbci.withclassification,
  ...
  remainmessage= sprintf('CSP/LAP global: %4.1f +/- %3.1f', ...
                          100*loss,100*loss_std);
  if ischar(opt.usedPat) & strcmpi(opt.usedPat,'auto'),
    proc= strukt('memo', {'csp_w', 'lap_w','sel_w'});
    ...
    proc.apply= ['fv= proc_linearDerivation(fv, [lap_w, sel_w*csp_w]); ' ...
                 'fv= proc_variance(fv); ' ...
                 'fv= proc_logarithm(fv); '];
    ...
    if ~isequal(opt.usedPat, 1:2*opt.nPat),
      proc.sneakin= {'global_w',global_w, 'sel_clab',sel_clab, 'clab_csp',opt.clab_csp};
      proc.train=['[fv_lap,lap_w]= proc_laplacian(fv, ''clab'',sel_clab); ' ...
                  '[fv,sel_w]= proc_selectChannels(fv, clab_csp); ' ...
