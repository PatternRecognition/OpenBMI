...
opt.nrClassify = length(opt.selival_opt)+1;
...
%             clear cnt_flt
    ...
    if isequal(opt.ival, 'auto') | isempty(opt.ival),
        ...
        opt.ival(1,:) = select_timeival(cnt_flt, mrk2, ...
            'do_laplace', opt.do_laplace);
        opt.ival(2:opt.nrClassify, :)= select_timeivalBylength(cnt_flt, mrk2, ...
...
%         remainmessage = '';
    ...
    bbci.weightVector = (1-cell2mat(loss))/sum(1-cell2mat(loss));
end  %% for ci
