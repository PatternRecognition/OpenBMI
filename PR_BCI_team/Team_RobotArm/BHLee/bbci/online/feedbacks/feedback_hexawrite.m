function opt = feedback_hexawrite(fig, opt, ctrl);
...
if opt.reset,
  ...
  [HH, hex]= feedback_hexawrite_init(fig, opt);
  [handles, H]= fb_handleStruct2Vector(HH);

  do_set('init',handles, 'hexawrite',opt);
  do_set(200);
...
if opt.changed==1,
  ...
  do_set(H.textfield, 'FontName','Courier New', opt.text_spec{:});
...
if state==10,  %% selection at step 2
  ...
  if written_char~=' ',
    ...
    do_set(H.textfield, 'String', fit_to_textfield(hex.written, hex));
  end
