function opt = feedback_cursor_arrow_blind(fig, opt, ctrl);
...
if opt.reset,
  ...
  [HH, cfd]= feedback_cursor_arrow_blind_init(fig, opt);
...
%    memo.x= max(-1-cfd.target_width, min(1+cfd.target_width, memo.x));
  ...
  if trial_terminates,
    ...
    if memo.trial>0,
      ...
      do_set(H.points(1), 'String',['getroffen: ' int2str(nHits)]);
      do_set(H.points(2), 'String',['daneben: ' int2str(nMisses)]);
      do_set(H.rejected_counter, 'String',['verworfen: ' int2str(memo.rejected)]);
...
if state==6,  %% wait before next trial starts (or game is over)
 ...
 %       state= 10;
    ...
    elseif opt.break_every>0 & memo.trial>0 & mod(memo.trial,opt.break_every)==0,
      ...
      memo.timer= 0;
      state= 1;
      opt = feedback_cursor_arrow_blind(fig, opt, ctrl);
      return;
    end
  else
    memo.timer= memo.timer + 1000/opt.fs;
...
if state==21,
  if memo.timer==0,
    ...
    do_set(H.points(1), 'String','Treffer : 0');
    do_set(H.points(2), 'String','Daneben : 0');
    do_set(H.rejected_counter, 'String','Verworfen: 0');
  end
