function opt = feedback_cursor_arrow_smr_nf(fig, opt, ctrl, smr);
%FEEDBACK_CURSOR_ARROW - BBCI Feedback: 1D Cursor Movement with Arrow Cues
%
%Synopsis:
...
% Modified version of feedback_cursor_arrow by S. Haufe & C. Maeder, April 2010
...
if opt.reset,
  ...
  [HH, cfd]= feedback_cursor_arrow_smr_nf_init(fig, opt);
...
if state==1,   %% show a blank screen with fixation cross
  ...
  smr = (smr-opt.smr_range(1))/(diff(opt.smr_range));
  if opt.smr_undershoot
    smr = 1-smr;
  end

  if memo.timer+memo.timeeps <= (opt.duration_blank + opt.duration_smr_wait)
    thresh = opt.smr_thresh;
  else
    thresh = opt.smr_thresh*(1-(memo.timer-(opt.duration_blank+opt.duration_smr_wait))/opt.duration_smr_descend);
  end

  if memo.timer+memo.timeeps >= opt.duration_blank & smr >= thresh,
    memo.timer= 0;
    state= 2;
  end
  memo.timer= memo.timer + time_since_lasttick;
...
if state==6,  %% wait before next trial starts (or game is over)
 ...
 %       state= 10;
    ...
    elseif opt.break_every>0 & memo.trial>0 & mod(memo.trial,opt.break_every)==0,
      ...
      memo.timer= 0;
      state= 1;
      opt = feedback_cursor_arrow_smr_nf(fig, opt, ctrl, smr);
      memo.timer= memo.timer + time_since_lasttick;
      return;
    end
  else
