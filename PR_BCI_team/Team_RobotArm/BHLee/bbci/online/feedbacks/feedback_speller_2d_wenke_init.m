function [hh,other_args] = feedback_speller_2d_wenke_init(fig, opt);
...
h_ordertext= text(opt.ordertext_pos(1), opt.ordertext_pos(2), ...
    untex(opt.ordertext(max(1,length(opt.ordertext)-opt.ordertext_len+1):end)), ...
                'fontUnits','normalized', ...
                'fontSize',opt.ordertext_fontsize, ...
                'FontWeight','bold','Color',[0 0 1]);
set([h_ordertext], 'horizontalAli','center', 'verticalAli','middle');
if strcmp(opt.orientation, 'portrait'),
  set([h_ordertext], 'rotation',90);
end
...
hh = [fig,gca,h_cursor,h_cou,h_pat,h_buffertext,h_text,h_boxtext',h_reg,h_ordertext];
