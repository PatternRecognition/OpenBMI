function opt = feedback_cursor_1d(fig, opt, ctrl);
...
%  timeout_policy: 'miss', 'lastposition', 'hitiflateral'
...
%
%See:
%  tester_cursor_1d, feedback_cursor_1d_init
...
if opt.reset,
  ...
  HH= feedback_cursor_1d_init(fig, opt);
  [handles, H]= fb_handleStruct2Vector(HH);
  
  do_set('init', handles, 'cursor_1d', opt);
  do_set(200);
...
if state==4,  %% move cursor until target was hit or time-out
  ...
  if abs(memo.x) > 1-2*opt.target_width | memo.trialwatch>opt.time_per_trial,
    if memo.trialwatch>opt.time_per_trial,
...
if state==5,  %% wait before next trial starts (or game is over)
  if memo.timer>opt.duration_show_selected,
    ...
    elseif opt.break_every>0 & mod(memo.trial,opt.break_every)==0,
      nHits= sum(memo.ishit(1:memo.trial));
      msg= sprintf('%d : %d', nHits, memo.trial-memo.rejected-nHits);
      do_set(H.msg, 'String',msg, 'Visible','on');
      memo.timer= 0;
      state= 6;
    else
