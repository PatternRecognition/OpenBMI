function send_informations(fig,typ,init)
...
%          1  send all informations + init signal
...
enable_stat = set_enable(fig,'off');drawnow;
...
if islogical(init) | isnumeric(init)
  if init
    backup_matlab_control_gui(setup);
  end
  % get the eval string by get_eval_string for the specific typ
  if strcmp(typ,'all') & init & setup.general.save
    startsaving(fig);
    setup = control_gui_queue(fig,'get_setup');  %% setup.savemode was updated by function startsaving
  end
  ...
  switch typ
   case 'all'
    ...
    if init
%    if ~obj_cmp(setup.general.setup_list1,setup.general.setup_list_default1) & length(setup.general.setup_list1)>0
...
%    if setup.general.player>1 & ~obj_cmp(setup.general.setup_list2,setup.general.setup_list_default2) & length(setup.general.setup_list2)>0
...
%%commented for 2 reasons: 1)the bias is set again at 0 if start playing_game
%%here, 2)it is better manually start playing_game when one is sure that
%%the subject is ready
% if setup.savemode == true & init
%   pause(2);
%   fprintf('Start feedback\n');
%   playing_game(fig,'all','play');
% end
...
% THIS FUNCTIONS EVALUATE IF A STRING MAKES SENSE
does_it_eval = false;
