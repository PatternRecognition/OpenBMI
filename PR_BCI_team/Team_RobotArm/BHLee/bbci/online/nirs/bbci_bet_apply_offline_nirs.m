...
%  bbci.control_port = 12471-bbci.player;
  ...
  for ii = 1:length(cont_proc)
    if ~isfield(cont_proc(ii),'use_state_var') | isempty(cont_proc(ii).use_state_var),
      use_state_var = [];
...
%  [cls, feature, cont_proc, post_proc, bbci] = adaptation(cls, feature, cont_proc, post_proc,  bbci, 'init');
  ...
  while loop
    ...
    markerToken= mrk.type(mrk_idx);
    markerDescr= mrk.desc(mrk_idx);
    ...
    [out,out_aa] = getClassifier('apply',size(data.x,1)*10/bbci.fs,cls);
    
     %%% Adapt the classifier, if necessary %%%    
    [cls, feature, cont_proc, post_proc, bbci] = adaptation(cls, feature, cont_proc, post_proc, bbci, timestamp);
    
    %%% if necessary perform mrk_out %%%
    mrkOut = performMarkerOutput('apply',marker_output,size(data.x,1)*10/bbci.fs,timestamp);
...
%%% log out %%%
    ...
    if isempty(bbci.fb_machine)
      ...
      drawnow;
    elseif strcmp(general_port_fields.feedback_receiver, 'pyff'),
