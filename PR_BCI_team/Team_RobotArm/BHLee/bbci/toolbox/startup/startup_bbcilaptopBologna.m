global DATA_DIR IO_ADDR IO_LIB
global TODAY_DIR REMOTE_RAW_DIR VP_CODE VP_SCREEN 
global TMP_DIR LOG_DIR
global ACQ_PREFIX_LETTER DROPBOX
global GTEC_SERIAL

%VP_SCREEN= [5 320 640 480];
VP_SCREEN= [-1279 0 1280 1024];
ACQ_PREFIX_LETTER= 'l';

DATA_DIR= 'E:\data\';
TMP_DIR= 'E:\tmp\';
LOG_DIR= 'E:\tmp\';
DROPBOX= 'E:\Dropbox\FSL_Study_Root\';
GTEC_SERIAL= 'UB-2009.10.20';

%parport= 'Docking';
%parport= 'USB2LPT';
parport= 'PCMCIA';
switch(parport)
  case 'USB2LPT',
    IO_ADDR= hex2dec('378');
  case 'Docking',
    IO_ADDR= hex2dec('3BC');
  case 'PCMCIA', 
    IO_ADDR= hex2dec('FFE8');
  otherwise,
    error('wrong setting for parport');
end
fprintf('Assuming Trigger cable is connected to %s. If not edit ''%s''.\n', parport, mfilename)

startup_bbci;
startup_new_bbci_online;
% setup_bbci_online;
IO_LIB= which('inpout32.dll');

% Run everything on this machine.
REMOTE_RAW_DIR = EEG_RAW_DIR;
set_general_port_fields('localhost');
%set_general_port_fields('bbcilaptop06b')

addpath([BCI_DIR '\public\gui\']);
addpath([BCI_DIR '\public\gui\protocols\']);
