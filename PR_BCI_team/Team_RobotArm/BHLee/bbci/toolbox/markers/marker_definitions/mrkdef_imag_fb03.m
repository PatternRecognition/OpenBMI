function mrk = mrkdef_imag_fb03(Mrk,file,opt);

%MRKDEF_IMAG_FB02 - prepare markers for extract marker information from 1D continue until touch feedback (feedback_cursor_1d_pro)
%
%Synopsis:
% MRK= mrkdef_imag_fb03(MRK,FILE,OPT)
%
%Arguments:
...
%       .istouch trials where bar at the correct side was touched before time out of the trial
...
%   touch_hit_left time when cursor correctly hits left bar for the first time
%   touch_hit_right time when cursor correctly hits right bar for the first time
%
...
opt.cursor_on = 0;
opt.adaptation = 1;
opt.markertypes = [1 2 11 12 21 22 25 41 42 60 70 71 200 210 220 230]
...
classDef = {1,2,11,12,21,22,25,41,42, 60,70,71,200, 210,211,212,213 ; 
            opt.classes_fb{:}, ...
            ['hit ' opt.classes_fb{1}], ['hit ' opt.classes_fb{2}], ...
            ['miss ' opt.classes_fb{1}], ['miss ' opt.classes_fb{2}], ...
            'reject','touch_hit_left' 'touch_hit_right'...
...
%ishit and is rejected and istouch
mrres = mrk_sortChronologically(mrk_selectClasses(mrk, 'remainclasses','hit*','miss*','reject', 'touch_hit_left','touch_hit_right'),0,'remainclasses');
...
mrres2 = mrk_sortChronologically(mrk_selectClasses(mrk2, 'remainclasses','hit*','miss*','reject','touch_hit_left','touch_hit_right'),0,'remainclasses');
...
mrk.ishit = []; mrk.free = []; mrk.isrejected =[]; 
mrk.istouch = []; mrk.duration = [];
mrk.adaptation_trial = [];

% for initialization only 
% steo  size of the for loop because "rotation off" is not interestng here
...
for i = 1:length(mrord.pos)
  ...
  %select the markers that belong to one run
  if mrfree.pos(pos3)<mrres.pos(pos2) & mrres.pos(pos2)<pos1 & ...
    ...
    mrk.ishit = [mrk.ishit, ([1, 1, 0 0 0 0 0]*mrres.y(:,pos2))>0];
    mrk.isrejected = [mrk.isrejected, ([0 0 0 0 1 0 0]*mrres.y(:,pos2))>0];
    mrk.istouch = [mrk.istouch, ([0 0 0 0 0 1 1 ]*mrres.y(:,pos2))>0];
...
mrk.indexedByEpochs = {'ishit','isrejected','istouch','free','duration','run_no','adaptation_trial','cursor_on'};
