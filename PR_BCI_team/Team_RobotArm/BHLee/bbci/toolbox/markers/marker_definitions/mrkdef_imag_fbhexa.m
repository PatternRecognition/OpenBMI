function mrk = mrkdef_imag_fbhexa(Mrk, file, opt);
...
for i = length(prop):-1:1
  if isempty(prop{i}) | flogf.update.pos(ind(i))<Mrk.pos(1)
    prop = prop(setdiff(1:length(prop),i));
    ind(i) = [];
  end
end

if size(flogf.initial{1},1)>=64
  str = find(strcmp(flogf.initial{1}{64,1},'String'));
  if ~isempty(str)
    prop = {flogf.initial{1}{64,2}{str},prop{:}};
    ind = [1,ind];
  end
end
