function [] = axespos(ax,axpos)

% AXESPOS - positions x/y axes of a 2D plot at an arbitrary position
%
%Usage:
% H = AXESPOS(<AX>,<AXPOS>)
%
%Input:
% H    : axes handle. If not provided, current axes (gca) is used.
% POS  : vector specifying the x/y coordinates of the
%        intersection of the axes; default [0 0].
%
% Matthias Treder
% Note: Most of the code provided here is taken from axescenter by Matt Fig.
%
...
% Created: 1/9/2009
% Modified by Matthias Treder 9/6/2009
...
if nargin<2    
    % If no position is provided, take origin
    axpos = [0 0];
end
%% Transform axes coordinates into figure coordinates
if ~strcmpi(get(gcf,'Units'),'normalized')
    % Does not use normalized units, switch units
    oldunits = get(gcf,'units');
    set(gcf,'units','normalized');
    [axpos(1) axpos(2)] = axescoord2figurecoord(axpos(1), axpos(2));
...
%% Comment out to preserve original color
%set([H{:}],{'color'},{'k'})
%% Determine position + size of new axes
ax_y = [axpos(1) D{1}(2) .01 D{1}(4)];
ax_x = [D{1}(1) axpos(2) D{1}(3) .01];

%% Next create our fake axes that will act as the center axes.    
S.a1 = axes('units',D{4},'posi',ax_y,...
     'ylim',D{3},'xtick',[],'handlev','of','tag','XAXIS','color','none',...
     'hittest','off','parent',D{5});  % Give this fake axis the props.
S.a2 = axes('units',D{4},'posi',ax_x,...
...
plistener(ax,'TickLength',@xytlen)   % add
...
plistener(ax,'XMinorTick',@xyxmint)
plistener(ax,'YMinorTick',@xyymint)
...
function [] = xytlen(varargin)
% Adjusts the tick length
ax = varargin{3};
S = getappdata(ax);
S = S.CENTERAXES;
tlen = get(ax,'TickLength');
set(S.a1,'TickLength',tlen);
set(S.a2,'TickLength',tlen);

function [] = xyxmint(varargin)
% Sets minor xticks on or off.
ax = varargin{3};
S = getappdata(ax);
S = S.CENTERAXES;
mint = get(ax,'XMinorTick');
set(S.a1,'XMinorTick',mint);
set(S.a2,'XMinorTick',mint);

function [] = xyymint(varargin)
% Sets minor yticks on or off.
ax = varargin{3};
S = getappdata(ax);
S = S.CENTERAXES;
mint = get(ax,'YMinorTick');
set(S.a1,'YMinorTick',mint);
set(S.a2,'YMinorTick',mint);