function [classy, E, S, P] = selectModel(dat, model, xTrials, ...
					 state)
...
% for questions or comments please send me an email or phone or
% visit me. Not everything is perfect but with comments or
% questions a lot can be improved
...
if length(xTrials)>=3 & xTrials(3)==-1,
  xTrials(3)= round((xTrials(2)-1)/xTrials(2)*sum(any(dat.y)));
end
...
if ~isfield(state,'divTr') & state.same
  if isfield(dat,'testset')
    [state.divTr, state.divTe] = sampleDivisions(dat.y, xTrials,[],dat.testset);
  else
...
[classy,E,S,P] = selectModel(dat,model,xTrials,state);
