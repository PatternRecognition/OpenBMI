function fv_fscore= proc_rfisherScore(fv, varargin)
...
if opt.tolerate_nans,
  me= [nanmedian(fv.x(:,cl1),2) nanmedian(fv.x(:,cl2),2)];
  va= [nanstd(fv.x(:,cl1),0,2).^2 nanstd(fv.x(:,cl2),0,2).^2];
else
  me= [median(fv.x(:,cl1),2) median(fv.x(:,cl2),2)];
  va= [var(fv.x(:,cl1)')' var(fv.x(:,cl2)')'];
end
md= me(:,1)-me(:,2);
fscore= (md.^2)./(va(:,1)+va(:,2)+eps);
if opt.preserve_sign,
  fscore= fscore .* sign(md);
end
