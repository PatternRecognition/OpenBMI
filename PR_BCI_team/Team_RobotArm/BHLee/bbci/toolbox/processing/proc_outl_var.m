function [fv, outlier_idx, dist1, dist2] = proc_outl_var(fv,varargin)
...
if sum([isdefault.trialthresh isdefault.trialperc isdefault.percentiles ...
       isdefault.trialgauss])<3,
  error('You may use only one of trialtresh, trialperc, trialgauss, and percentiles');
end
if ~isdefault.trialthresh,
  opt.percentiles= [];
end
...
for var_ind = 1:varianceNum
  ...
  if opt.trialgauss>0
    me = mean(dist1);
    st = var(dist1);
    opt.trialthresh(var_ind)= sqrt(st)*erfinv(1-2*opt.trialgauss)+me;
  end

  if ~isempty(opt.percentiles),
    perc= percentiles(dist1, opt.percentiles);
    opt.trialthresh(var_ind)= perc(2) + diff(perc) * opt.threshold_factor;
    outlier_idx= find(dist1 > opt.trialthresh(var_ind));
  elseif opt.trialthresh(var_ind)>0
  ...
  if opt.display
    % generate plots.
    %subplot(2,1,1);
    figure(opt.handles(2*var_ind-1)); clf;
    if isempty(opt.percentiles),
      perc_disp= [20 80];
    else
      perc_disp= opt.percentiles;
    end
    boxplot(dist1, 'notch',1, ...
            'whiskerpercentiles',perc_disp, ...
            'whiskerlength',opt.threshold_factor);
    xlabel('');
    hold on;
    plot(linspace(1.15, 1.45, length(dist1)), sort(dist1));
    set(gca, 'XLim', [0.9 1.5], 'XTick',[]);
    if  ~isempty(opt.percentiles),
      line([1.15 1.15; 1.45 1.45], [1;1]*perc, 'Color','k', 'LineStyle','--');
    end
    line([1.15 1.45], [1 1]*opt.trialthresh(var_ind), 'Color','r');
    title('trial variance distances');
    if opt.classwise
