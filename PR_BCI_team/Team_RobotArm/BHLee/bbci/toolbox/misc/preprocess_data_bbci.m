function opt= preprocess_data_bbci(sub_dir, varargin)
%preprocess_data_bbci - Convert data sets from generic data format to matlab
%
%Synopsis:
% preprocess_data_bbci(SUB_DIR)
% preprocess_data_bbci(SUB_DIR, 'Property',Value, ...)
%
%Arguments:
...
[mrk_orig, fs_orig]= eegfile_readBVmarkers(file, 0);

ft = file_type;
while length(ft)>0 & ~exist(['mrkodef_' ft]),
  ft = ft(1:end-1);
end
if length(ft)>0,
  fprintf('Using ''mrkodef_%s'' for marker definition.\n', ft); 
  mrk= feval(['mrkodef_' ft], mrk_orig, opt);
  ...
  if strpatterncmp('imag_lett*',file_type) | ...
...
% BB: ? for processing='cut50', data is loaded twice ?
