function bbci_bet_apply(setup_list,varargin)
%!! THIS FUNCTION IS MODIFIED TO WORK WITH THE CAPACITIVE ELECTRODES !!
%[in construction]
%
...
%     .host   the name of the brainvision computer [brainamp]
...
%
% $Id: bbci_bet_apply.m,v 1.5 2006/10/17 17:00:14 neuro_cvs Exp $
...
%  bbci.control_port = 12471-bbci.player;
  ...
  bbci = set_defaults(bbci,'fb_machine',general_port_fields(1).bvmachine,'fb_port',general_port_fields(min(bbci.player,length(general_port_fields))).control{2},'other_clients',{},'other_ports',[]);
  ...
  % Open the TCP port to BRAINVSION
  acquire_state_bs = acquire_bv_braunschweig(bbci.fs, bbci.host);
  acquire_state = acquire_bv(bbci.fs,bbci.host);
...
% $$$   acquire_state.clab = allChans;
  ...
  storeContData('init', length(cont_proc), length(acquire_state_bs.chan_sel),bbci,'sloppyfirstappend',1);
  
  
  % For each processing, compute the actual channel indices from the string
  % labels
  for pr = 1:length(cont_proc)
    cont_proc(pr).chans = chanind(acquire_state_bs.clab, cont_proc(pr).clab);
  ...
  ind = strfind(bbci.save_name,'/');
  subdir = bbci.save_name((ind(end-1)+1):ind(end));
  matlab_opt = struct('fs',bbci.fs,'nChans',length(cont_proc.clab),...
		      'subdir',subdir);
  eegfile_onlinesaveBV('init',matlab_opt, acquire_state_bs);
  ...
  while loop
    ...
    while size(data, 1)<(bbci.minDataLength/1000*bbci.fs),
      ...
      [currData, block] = ...
           acquire_bv_braunschweig(acquire_state_bs);
      [bvData, dmyBlock, markerPos, markerToken, markerDescr] = ...
      ...
      eegfile_onlinesaveBV('store',currData,bvData,markerPos,markerToken);
    ...
    for pr = 1:length(cont_proc)
      ...
      for i = 1:length(theProc.procFunc),
        if nargout(theProc.procFunc{i})==1
