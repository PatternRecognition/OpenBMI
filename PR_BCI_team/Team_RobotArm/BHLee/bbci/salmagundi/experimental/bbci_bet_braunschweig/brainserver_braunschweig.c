...
#include <sys/time.h>
#include <unistd.h>
#include "../bbci_bet_unstable/winunix/winthreads.h" /* These are some portability layers */
#include "../bbci_bet_unstable/winunix/winevents.h"  /* for WinThreads and Events */
...
#include "headerfifo_braunschweig.h"
...
/*  struct RDA_MessageData data;    header.nType = 2 */
  struct RDA_MessageData32 data;   /* header.nType = 4 */
		   ...
		   struct RDA_MessageStart **pMsgStart)
{
  ...
  *pMsgStart = 0;
  ...
  if ((he = gethostbyname(bv_hostname)) == NULL) {
      mexWarnMsgTxt("acquire_bv_braunschweig: gethostbyname failed.");
  ...
  if ((brainserver_socket = socket(PF_INET, SOCK_STREAM, 0)) == -1) {
      mexWarnMsgTxt("acquire_bv_braunschweig: Couldn't open socket.");
      CLOSESOCKET(brainserver_socket);
      return -3;
  }
...
int getData(struct RDA_MessageData32 **pMsgData, int *blocksize, 
	    int lastBlock, ULONG nChannels)
{
  ...
  while (waiting) {
    ...
    if (nResult > 0) {
      mexPrintf(" {{ getData: %d }}\n", nResult);
      if (pHeader->nType == 4) { 
        *pMsgData = (struct RDA_MessageData32*)pHeader;
  ...
  if (failed) {
    ...
    if(nResult == -1) {
       ...
       */        
        ...
        int structSize = sizeof(struct RDA_MessageData32);
	/*  - sizeof(short) - sizeof(struct RDA_Marker); */
        ...
        for(n = fifo.last; n; n = n->prev) {
            pHeader = n->payload;
            if(pHeader->nType == 4) {
                pmd = (struct RDA_MessageData32*)pHeader;
                nPoints += pmd->nPoints;
                *blocksize = pmd->nPoints;
/*                nMarkers += pmd->nMarkers;        */
                nBlock = pmd->nBlock;

                structSize += sizeof(float)*nChannels*pmd->nPoints;
/*                for(m = 0; m < pmd->nMarkers; m++) {
                    struct RDA_Marker *pma = getMarker(pmd, nChannels, m);
...
/*       (*pMsgData)->nMarkers = nMarkers;  */
        ...
        {
            char *base = (char*)(*pMsgData)->fData;
            float *pdst = (float*)base;
/*            struct RDA_Marker *mdst = 
                (struct RDA_Marker *)(base + sizeof(float)*nChannels*nPoints); */
            ...
            while(pHeader = headerFIFOpop(&fifo)) {
                if(pHeader->nType == 4) {
                    ...
                    psize = nChannels*pmd->nPoints;
                    memcpy(pdst, pmd->fData, sizeof(float)*psize);

/*                    pdst += psize;

                    for (m = 0; m < pmd->nMarkers; m++) {
                        struct RDA_Marker *pm = getMarker(pmd, nChannels, m);
...
struct RDA_Marker *getMarker(struct RDA_MessageData32 *pmd, 
			     int nChannels, int idx)
{
    if (0 <= idx && idx < pmd->nMarkers) {
        struct RDA_Marker *pm 
	  = (struct RDA_Marker *)(pmd->fData + pmd->nPoints * nChannels);
        for(;idx; idx--) {
...
DWORD WINAPI pollThread(LPVOID lpParameter)
{
  ...
  while(1) {
    ...
    if (result > 0) {
      if (header->nType == 4) {
	int block = ((struct RDA_MessageData32 *)header)->nBlock;
