...
for vp= 1:length(subdir_list),
  ...
  fv= proc_variance(fv);
  fv= proc_logarithm(fv);
  out1= applyClassifier(fv, 'LSR', C);
  fv.x = out1;
  [divTr,divTe] = sample_chrontrial(fv.y,50,50,S.mrk.trial_no);
  opt_xv = strukt('divTr',divTr,'divTe',divTe);
  [loss, dum, out2] = xvalidation(fv, 'LSR', opt_xv);
  
  out = out1;;
  for ii = 1:length(divTe)
    out(divTe{ii}{1}) = [ out2(1,divTe{ii}{1},ii)];
  end
  memo{vp}.out = out;
  memo{vp}.label = fv.y;
...
save([res_dir 'csp_bias'], 'perf', 'stat', 'subdir_list','memo');
