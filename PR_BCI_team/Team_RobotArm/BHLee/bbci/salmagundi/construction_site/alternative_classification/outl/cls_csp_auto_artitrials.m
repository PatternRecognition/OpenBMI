...
%  [mrk_clean, dmy, nfo]= reject_varEvents(cnt, mrk_train, [500 4500]);
%  fprintf('rejected: [%d/%d/%d] -> %d trials.\n', nfo, ...
%          length(mrk_train.pos)-length(mrk_clean.pos));
  [mrk_clean, dmy, dmy, nfo]= ...
      reject_varEventsAndChannels(cnt, mrk_train, [500 4500]);
  fprintf('rejected: /%s/ -> %d trials.\n', ...
...
save([res_dir 'csp_auto_rev'], 'perf', 'memo', 'subdir_list');
