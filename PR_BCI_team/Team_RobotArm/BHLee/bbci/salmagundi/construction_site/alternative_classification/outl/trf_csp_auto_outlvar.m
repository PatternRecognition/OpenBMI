...
for vp= 1:length(subdir_list),
  ...
  csp_ival= select_timeival(cnt_flt, mrk);
  clear cnt_flt
  csp_band= select_bandnarrow(cnt_lap, mrk, csp_ival);
  clear cnt_lap
  [filt_b,filt_a]= butter(5, csp_band/cnt.fs*2);
  cnt= proc_filt(cnt, filt_b, filt_a);
  csp_ival= select_timeival(cnt, mrk);
  fv= makeEpochs(cnt, mrk, csp_ival);
  [fv, outl_idx]= ...
      proc_outl_var(fv, 'log',1, display',1, 'percentiles',[20 80], ...
                    'handles',1:2);
  drawnow;
...
save([res_dir 'csp_auto_outlvar'], 'perf', 'memo', 'subdir_list');
