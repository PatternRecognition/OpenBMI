...
for vp= 1:length(subdir_list),
  ...
  mk= mrk_train;
  %% select time interval and frequency band for CSP
  [filtbb_b,filtbb_a]= butter(5, [4 35]/cnt.fs*2);
  cnt_bb= proc_filt(cnt, filtbb_b, filtbb_a);
  cnt_lap= proc_laplace(cnt_bb);
  csp_ival= select_timeival(cnt_lap, mk);
  clear cnt_flt cnt_lap
  cnt_lap= proc_laplace(cnt);
  csp_band= select_bandnarrow(cnt_lap, mk, csp_ival);
%  csp_band= select_bandbroad(cnt_lap, mk, csp_ival);
  clear cnt_lap
  [filt_b,filt_a]= butter(5, csp_band/cnt.fs*2);
  cnt_flt= proc_filt(cnt, filt_b, filt_a);
  csp_ival= select_timeival(cnt_flt, mk);
  %% calculated CSP on selected frequency band and time interval
  fv= makeEpochs(cnt_flt, mk, csp_ival);
  [dmy, csp_w]= proc_csp3(fv, 'patterns',3, 'scaling','maxto1');
  %% now apply AR estimation to broad band signal
  fv= proc_linearDerivation(cnt_bb, csp_w, 'prependix','csp');
  fv= makeEpochs(fv, mk, csp_ival);
  fv= proc_spectrum(fv, csp_band, 'db_scaled', 1);
  fv.x=log(mean(fv.x));
  ...
  cnt= proc_filt(cnt, filtbb_b, filtbb_a);
...
save([res_dir 'csp_auto_welch'], 'perf', 'memo', 'subdir_list');
