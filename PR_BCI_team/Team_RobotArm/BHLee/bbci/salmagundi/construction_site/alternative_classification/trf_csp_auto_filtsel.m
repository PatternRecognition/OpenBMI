...
for vp= 1:length(subdir_list),
  ...
  [mrk_clean, rClab]= reject_varEventsAndChannels(cnt, mrk, [500 4500]);
%  fprintf('rejected: %d trials and channels <%s>. ', ...
%          length(mrk.pos)-length(mrk_clean.pos), vec2str(rClab));
...
save([res_dir 'csp_auto_filtsel'], 'perf', 'memo', 'subdir_list');
