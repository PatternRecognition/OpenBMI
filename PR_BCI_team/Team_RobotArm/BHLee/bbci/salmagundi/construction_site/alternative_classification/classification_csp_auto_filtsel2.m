...
%% for proc_csp_filtsel ...
addpath([BCI_DIR 'studies/season2'])
...
for vp= 1:length(subdir_list),
  ...
  mk= mrk_train;
  [mk_dmy , rClab]= reject_varEventsAndChannels(cnt, mrk_train, [500 4500]);
%  fprintf('rejected: %d trials and channels <%s>.\n', ...
%          length(mrk_train.pos)-length(mk.pos), vec2str(rClab));
  fprintf('rejected channels <%s>.\n', vec2str(rClab));
  clab= setdiff(cnt.clab, rClab);
  cnt= proc_selectChannels(cnt, clab);
  ...
  csp_ival= select_timeival(cnt, mk);
  fv= makeEpochs(cnt, mk, csp_ival);
%  [fv, csp_w]= proc_csp3(fv, 'patterns',3);
  csp_opt= struct('patterns', 2, ...
                  'score', 'fisher', ...
                  'selectPolicy', 'maxvalueswithcut');
  [fv, csp_w]= proc_rcsp(fv, csp_opt);
  fv= proc_variance(fv);
...
save([res_dir 'csp_auto_filtsel2'], 'perf', 'memo', 'subdir_list', 'csp_opt');
