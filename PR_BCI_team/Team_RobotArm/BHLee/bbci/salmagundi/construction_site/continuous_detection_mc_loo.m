...
classy = {'multiClass',[1 0;0 1;-1 -1],inline('-x','x'),'LSR'};
E= trunc(-800:10:800);
tubePercent= [5 10 15];
...
for is= 1:nShifts,
  ...
  eval(proc);
  for ie= 1:nEvents,
    out_mc= feval(applyFcn, C(ie), fv.x(:,ie));
    outTraces(is,ie)= out_mc(3);
  end
...
save testTraces_mn_mc outTraces
