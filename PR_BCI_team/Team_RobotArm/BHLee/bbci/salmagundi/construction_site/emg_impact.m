...
%fig_file= 'divers/emg_impact/good_vs_bad'
...
%  badEMG= [badLeft badRight];
  ...
  fv_emg.y= mrk.y;
  errEMG(is,:)= doXvalidation(fv_emg, 'FisherDiscriminant', xTrials, 1);
  fprintf('emg_on_gut@%5d> ', E(is));
  fv_emg.y= yog;
  errEMGog(is,:)= doXvalidation(fv_emg, 'FisherDiscriminant', xTrials, 1);
  fprintf('emg_on_bad@%5d> ', E(is));
  fv_emg.y= yob;
  errEMGob(is,:)= doXvalidation(fv_emg, 'FisherDiscriminant', xTrials, 1);

  fprintf('eeg_on_all[%4d]> ', nEvents);
  fv_eeg.y= mrk.y;
  errEEG(is,:)= doXvalidation(fv_eeg, 'FisherDiscriminant', xTrials, 1);
  fprintf('eeg_on_gut[%4d]> ', nGood(is));
  fv_eeg.y= yog;
  errEEGog(is,:)= doXvalidation(fv_eeg, 'FisherDiscriminant', xTrials, 1);
  fprintf('eeg_on_bad[%4d]> ', nBad(is));
  fv_eeg.y= yob;
  errEEGob(is,:)= doXvalidation(fv_eeg, 'FisherDiscriminant', xTrials, 1);

end
%clear cnt_emg cnt_eeg

save eeg_vs_emg E errEEG errEEGog errEEGob errEMG errEMGog errEMGob nGood nBad
...
if exist('fig_file', 'var'),
  addTitle([untex(cnt.title) ...
            ': classification on ''good''- and ''bad''-EMG subsets']);
  saveFigure(fig_file);
end
...
lp= -170;
epo= makeSegments(cnt_emg, mrk, lp + [-300 0]);
fv_emg= proc_detectEMG(epo);
C= train_FisherDiscriminant(fv_emg.x, fv_emg.y);
out= C.w'*fv_emg.x+C.b;
...
showClassDistr(out, left, right, 55);
yLim= get(gca, 'yLim');
hl= line(ones(2,1)*out([goodLeft(end) goodRight(1)]), [yLim' yLim']);
set(hl, 'color','k', 'lineStyle','-.', 'lineWidth',1.5);
xlabel('projection on (EMG-) Fisher Discriminant');
title(['determination of ''good''- and ''bad''-EMG trials at ' ...
       int2str(lp) ' ms']);
%saveFigure('divers/emg_impact/subset_determination')

epo= makeSegments(cnt_eeg, mrk, lp + [-1300 0]);
fv= proc_filtBruteFFT(epo, [0.8 2], 128, 150);
fv= proc_jumpingMeans(fv, 5);

fprintf('on good EMGs\n');
fprintf('emg: ');
fv1_emg= setGoal(fv_emg, {goodLeft, goodRight});
doXvalidation(fv1_emg, 'FisherDiscriminant', [10 10]);
fprintf('eeg: ');
fv1= setGoal(fv_eeg, {goodLeft, goodRight});
doXvalidation(fv1, 'FisherDiscriminant', [10 10]);

fprintf('\non bad EMGs\n');
fprintf('emg: ');
fv2_emg= setGoal(fv_emg, {badLeft, badRight});
doXvalidation(fv2_emg, 'FisherDiscriminant', [10 10]);
fprintf('eeg: ');
fv2= setGoal(fv_eeg, {badLeft, badRight});
doXvalidation(fv2, 'FisherDiscriminant', [10 10]);
...
epic= makeSegments(cnt_emg, mrk, [-760 60]);

figure(2);
[so,si]= sort(out);
k= 0;
k= k+1,
showEvent(epic, si(k), mrk);
yLim= get(gca, 'yLim');
hl= line([lp lp], yLim);
set(hl, 'color','k', 'lineStyle',':');
100*mean(sign(out(si(1:k)))~=[-1 1]*fv.y(:,si(1:k)))
100*mean(sign(out(si(k-9:k)))~=[-1 1]*fv.y(:,si(k-9:k)))
