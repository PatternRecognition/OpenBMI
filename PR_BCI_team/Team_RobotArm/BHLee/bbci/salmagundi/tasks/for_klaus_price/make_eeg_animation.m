...
video= set_defaults([], ...
                    ...
                    'save_name', 'eeg_animation_%s.avi', ...
                    'maxFileSize',1000000000*2,...
                    'compress', 1, ...
                    'codec_spec', '-use_rgb -z -y xvid -R3 -w1');

anim= set_defaults([], ...
                   'eeg_clab', {'F7,3,z,4,8', 'FC5,3,z,4,6', 'T7', ...
                                'C5-6','T8','CP5,3,z,4,6','P7,3,z,4,8'}, ...
                   'eeg_band', [2 45], ...
                   'eeg_scale', [], ...
...
load([save_dir 'csp_' sd], 'csp_band','mrk');
...
if isempty(anim.eeg_scale),
  ...
  pp= percentiles(ct.x(:), [0.05 99.95]);
  anim.eeg_scale= 1.5*max(abs(pp));
  clear ct
end
...
h_eeg_map= subplotxl(1, 1, 1, [0.05], [0.01 0 0.61]);


%% plot whole traces of which on small windows will be visible at time
T= size(cnt.x,1);

h_eeg_ax= subplotxl(1, 1, 1, [0.05], [0.4 0 0]);
...
if isempty(anim.window_length),
  set(h_eeg_ax, 'Unit','Pixel');
  pos= get(h_eeg_ax, 'Position');
  pos([1 3])= round(pos([1 3]));
  set(h_eeg_ax, 'Position',pos);
  winlen_sa= pos(3);
else
...
for ii= 1:length(mrk.pos),
  ...
  if anim.show_class_tags,
    h_tag(ii,ax)= text(xx(1), yLim(2), tag);
...
if video.save,
  movienr= 1;
  movie= avifile([video.save_dir 'eeg_tmp'], ...
...
for ff= [1:nFrames] + offset,
  ...
  set([h_eeg_scalp.head h_eeg_scalp.nose], ...
      ...
      'LineWidth', anim.eeg_outline_width);
  ...
  if video.save,
    ...
    if sz>=video.maxFileSize | ff==nFrames+offset,
      movie= close(movie);
      movie_name= sprintf('eeg_tmp_%03d.avi',movienr);
      if video.compress,
        cmd= sprintf('cd %s; transcode -i eeg_tmp.avi %s -o %s',...
                     video.save_dir, video.codec_spec, movie_name);
        unix_cmd(cmd, 'using transcode');
      else
        cmd= sprintf('cd %s; mv eeg_tmp.avi %s', video.save_dir, ...
      ...
      if ff<nFrames+offset,
        movienr= movienr + 1;
        movie= avifile([video.save_dir 'eeg_tmp.avi'], ...
                       'Compression','none', 'Quality',100, 'fps',video.fps);
      end
    end
  else
...
if video.save,
  ...
  if movienr>1,
    cmd= sprintf('cd %s; avimerge -o %s -i eeg_tmp_???.avi', ...
                 video.save_dir, movie_name);
    unix_cmd(cmd, 'merging AVI files');
    cmd= sprintf('cd %s; rm -f eeg_tmp_???.avi', video.save_dir);
    unix_cmd(cmd, 'deleting temp files');
  else
    cmd= sprintf('cd %s; mv eeg_tmp_001.avi %s', video.save_dir, movie_name);
    unix_cmd(cmd, 'renaming video file');
  end
  cmd= sprintf('cd %s; rm -f eeg_tmp.avi', video.save_dir);
%  unix_cmd(cmd, 'deleting temp files');
end
