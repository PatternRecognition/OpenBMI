...
%x= x/top*0.95;
%save([EEG_EXPORT_DIR tag '_pc_multiscale.txt'], 'x', '-ASCII');
...
rcms= rcms/top*0.95;
save([EEG_EXPORT_DIR tag '_multiscale.txt'], 'rcms', '-ASCII');

cmd= sprintf('cd %s; python convert_multiscale_data.py %s', pydir, tag);
[s,w]= unix(cmd);
if s,
  fprintf('error: %s', w);
end
