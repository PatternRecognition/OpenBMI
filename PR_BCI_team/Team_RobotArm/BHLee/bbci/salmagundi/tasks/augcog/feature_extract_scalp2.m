...
for fi = 1:length(fil);
  for ta = 1:length(task);
    ...
    for ba = 1:size(band,1)
      try
        fv = get_augcog_bandPowerbyvariance(augcog(fil(fi)).file,task{ta},[2000 20000],band(ba,:),'car',{'not','E*','M*','Fp*'});
        model = {'LPM',10000};
      ...
      try
        fv = get_augcog_bandPowerbyvariance(augcog(fil(fi)).file,{['high ' task{ta}(2:end)],'low drive'},[2000 20000],band(ba,:),'car',{'not','E*','M*','Fp*'});
      catch
      lasterr
      end
...
%    set(hh,'FontSize',0.5*get(hh,'FontSize'));
    saveFigure(['augcog_misc/feature_extraction2_LPM_band_' augcog(fil(fi)).file '_' task{ta}(2:end)],[20 30]);
...
%    set(hh,'FontSize',0.5*get(hh,'FontSize'));
    saveFigure(['augcog_misc/feature_extraction2_LPM_allband_' augcog(fil(fi)).file '_' task{ta}(2:end)],[20 30]);
