function [fv,apply] = get_augcog_spectrum(blk,varargin);
%GET_AUGCOG_SPECTRUM OPENS AN AUGCOG FILE AND CALCULATES SPECTRAL INFORMATION
%
% usage:
%     fv = get_augcog_spectrum(file,<task=all available,ivallen=1000,band=[3 40],spatial='diagonal',channels={'not','E*','M*'});
%     fv = get_augcog_spectrum(blk,<task=all available,ivallen=1000,band=[3 40],spatial='diagonal',channels={'not','E*','M*'});
%     fv = get_augcog_spectrum(cnt,mrk,<ivallen=1000,band=[3 40],spatial='diagonal',channels={'not','E*','M*'});
%
% input:
...
% spectrum
fv = proc_spectrum(epo,opt.band(1,:));
str = sprintf('fv=proc_spectrum(epo,[%i %i]);',opt.band(1,:));
for i = 2:size(opt.band,1)
  f = proc_spectrum(epo,opt.band(i,:));
  fv = proc_catFeatures(fv,f);
  str = sprintf('%s hlp_fv2 = proc_spectrum(epo,[%i %i]);fv=proc_catFeatures(fv,help_fv2);',str,opt.band(i,:));
end
