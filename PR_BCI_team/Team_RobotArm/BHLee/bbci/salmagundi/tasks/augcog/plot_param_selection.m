...
for i = 1:length(spatial)
  spale = max(spale,length(spatial{i}));
end
pale = 0;
for i = 1:length(params)
  pale = max(pale,length(params{i}));
end

result = zeros(length(fil),length(task),length(feature),length(ival),length(freq),spale,pale,length(channels),length(ival_apply),length(CL),length(modell));
for fi = 1:length(fil)
  clear traces;
...
%                        cha = convert_rhomb(channels{ch},'>');
%                        str = sprintf('%s_',cha{:});
str = sprintf('%s_',channels{ch}{:});
str = sprintf('%s_%s%s_%s_%s_%i_%i_%i_%s_%i_%i_%s_%s',augcog(fil(fi)).file,CL{cl}{1}(1:end-1),CL{cl}{2}(1:end-1),task{ta}(2:end),feature{fe},ival{iv},freq{fr}(1),freq{fr}(2),spatial{fe}{sp}{1},pa,ival_apply{iva},modename,str(1:end-1));
                        
                        try 
                          S = load(['/home/tensor/dornhege/calcData/param_select/' str]);
                        
                          result(fi,ta,fe,iv,fr,sp,pa,ch,iva,cl,model) = plot_ongoing_classification(S.traces);
                          cha = convert_rhomb(channels{ch},'>');
                          str = sprintf('%s_',cha{:});
                          str = sprintf('%s_%s%s_%s_%s_%i_%i_%i_%s_%i_%i_%s_%s',augcog(fil(fi)).file,CL{cl}{1}(1:end-1),CL{cl}{2}(1:end-1),task{ta}(2:end),feature{fe},ival{iv},freq{fr}(1),freq{fr}(2),spatial{fe}{sp}{1},pa,ival_apply{iva},modename,str(1:end-1));

                          saveFigure(['augcog_misc/param_select_' str],[10 7]);
                        end
                      end
...
save('/home/tensor/dornhege/calcData/param_select/all_results','fil','task','feature','ival','freq','spatial','params','channels','ival_apply','CL','modell','result');