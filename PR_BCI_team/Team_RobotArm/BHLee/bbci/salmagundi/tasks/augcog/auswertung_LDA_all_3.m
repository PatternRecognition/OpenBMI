...
fid = fopen('/home/tensor/dornhege/neuro_cvs/Tex/bci/augcog_misc/results_LDA_all_3.tex','w');
...
fprintf(fid,'\\title{Results of LDA classification, Season 3}\n');
    ...
    for ta = 1:length(task)
      if size(test,1)>=ta & sum(test(ta,:)~=0)>0
        ...
        for fe = 1:length(feature);
          ...
          fprintf(fid,'PROC (wl/fr/sp) & base vs. low & base vs. high & low vs. high');
          fla = '\hline';
            ...
            for fr = 1:length(freq)
              switch feature{fe}
               case 'csp'
                ...
                tr = train(ta,fe,iv,fr,1,1,1,:);
                stt = stantrain(ta,fe,iv,fr,1,1,1,:);
                for i = 1:3
                  fprintf(fid,' & %2.1f \\std{%1.1f} (%2.1f \\std{%1.1f})',te(i),st(i),tr(i),stt(i));
                end
...
% $$$                 end
               ...
               otherwise
                for sp = 1:length(spatial{fe})
                  ...
                  tr = train(ta,fe,iv,fr,sp,1,1,:);
                  stt = stantrain(ta,fe,iv,fr,sp,1,1,:);
          ...
          fprintf(fid,'\n\\end{tabular}\n');
          fprintf(fid,'\\caption{Classification results for $%s$, %s and feature $%s$}\n',augcog(fil(fi)).file,task{ta}(2:end),feature{fe});
