...
file_fs = 500;
for phase = 1:file_fs/opt.sampling_fs;
  from = (phase-1)/file_fs*1000;

  writelogfile(0,sprintf('classifier_sim_mean_%sT%s_phase_%i.log',s(1:8),s(9:14),phase-1),opt.sampling_fs);
...
cnt = readGenericEEG([di file],[],opt.sampling_fs,from,inf,1);
