function fv= makeLetterTrials_colrow(fv_col, fv_row)
%fv= makeLetterTrials_colrow(fv_col, fv_row)

nTrialsPerColrow= 6;
lett_matrix= reshape(['A':'Z', '1':'9', ' '], 6, 6)'; 

[nF_col, nC_col, nTrials]= size(fv_col.x);
[nF_row, nC_row, nTrials]= size(fv_row.x);

lett= unique(fv_col.base);
nLett= length(lett);
nRep= nTrials/(nTrialsPerColrow*nLett);

fv_col.x= reshape(fv_col.x, [nF_col*nC_col, nTrialsPerColrow, nRep, nLett]);
fv_row.x= reshape(fv_row.x, [nF_row*nC_row, nTrialsPerColrow, nRep, nLett]);
