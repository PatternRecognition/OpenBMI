...
for si= 1:length(sbj_code),
  ...
  for n= 1:nSub,
    ...
    if length(tmp_y)> 300000,
      ...
      err_tmp1= nanmean((true_y-est_y).^2);
      err_tmp2= nanmean((true_y+est_y).^2);
      err_tmp= min(err_tmp1, err_tmp2);
      err(si,n)= err_tmp;
    else
      est_y= zeros(T100, 1);
      T0= min(T100, length(tmp_y));
      est_y(1:T0)= tmp_y(1:T0);
      err_tmp1= nanmean((true_y100-est_y).^2);
      err_tmp2= nanmean((true_y100+est_y).^2);
      err_tmp= min(err_tmp1, err_tmp2);
      err(si,n)= err_tmp;
    end
