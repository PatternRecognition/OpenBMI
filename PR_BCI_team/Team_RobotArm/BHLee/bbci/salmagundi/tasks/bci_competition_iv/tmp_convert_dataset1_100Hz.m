...
hdr= eegfile_readBVheader(filein);
Wps= [42 49]/hdr.fs*2;
[n, Ws]= cheb2ord(Wps(1), Wps(2), 3, 40);
[filt.b, filt.a]= cheby2(n, 50, Ws);

%% 100Hz Version
%[ct, mrk_orig]= eegfile_readBV([filein '*'], 'fs',100, 'filt',filt, ...
[ct, mrk_orig]= eegfile_loadBV([filein '*'], 'fs',100, 'filt',filt, ...
...
fileout= [folder savelist{ff} appendix(vp)];
if ff==1,
  save(fileout, 'cnt','mrk','nfo',opt_save{:});
  save_ascii([fileout '_mrk'], [mrk.pos; mrk.y]');
else
  save(fileout, 'cnt','nfo',opt_save{:});
  save_ascii([DATA_DIR 'bci_competition_iv/datasets1/' ...
              savelist{ff} appendix(vp) '_mrk'], ...
             [mrk.pos; mrk.y]');
end
