...
opt.test = true;
opt.speakerCount = 2;
opt.speakerSelected = [1 2 3 4 5 6]; % first is lowest tone, last highest.
...
SpatialProfile = 'TNO';
tmp = wavread([SOUND_DIR 'SpatialCues\' SpatialProfile '1.wav']);
opt.cueStream = zeros([length(opt.speakerSelected) size(tmp,1) size(tmp,2)]);

for i = 1:length(opt.speakerSelected),
    speaker = opt.speakerSelected(i);
    opt.cueStream(speaker,:,:) =  wavread([SOUND_DIR 'SpatialCues\' SpatialProfile int2str(speaker) '.wav']);
end
