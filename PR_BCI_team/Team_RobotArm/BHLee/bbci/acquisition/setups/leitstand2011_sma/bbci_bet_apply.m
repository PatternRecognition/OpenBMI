...
% global values for the acquire function
global acquire_func
...
%  fprintf([escape_printf(new_setup) '\n\n']);
  ...
  for pr = 1:length(cont_proc),
    ...
    if ~isfield(cont_proc(pr),'procFunc') || isempty(cont_proc(pr).procFunc)
  ...
  if isfield(bbci, 'filt') && ~isempty(bbci.filt)
    acquire_state = acquire_func(bbci.fs,general_port_fields(1).bvmachine, bbci.filt.b, bbci.filt.a);
  else
    acquire_state = acquire_func(bbci.fs,general_port_fields(1).bvmachine);
  end
...
%%% log out %%%
    ...
    if isempty(bbci.fb_machine)
      ...
      thistime= clock;
      %% evaluate classifier only if > 2s have passed since last
      if ~isnan(udp(1)) & etime(thistime,lasttime)>2,
        lasttime= thistime;
        if udp(1)<0,
          out_signal= 2;
        else
        ...
        send_xml_udp('i:bbci_act_output', out_signal);
        ppTrigger(out_signal)
        fprintf('out: %d, udp: %g\n', out_signal, udp(1));
      end
    end
    ...
    if ~isempty(intersect(toe, bbci.start_marker)),
      fprintf('[bbci_bet_apply:] Start marker received.\n');
      start_marker_received= 1;
    end
    if ~isempty(intersect(toe, bbci.quit_marker)) & start_marker_received,
      fprintf('[bbci_bet_apply:] Quit marker received.\n');
      loop= false;
      run= false;
      pause(bbci.pause_after_quit_marker/1000);
