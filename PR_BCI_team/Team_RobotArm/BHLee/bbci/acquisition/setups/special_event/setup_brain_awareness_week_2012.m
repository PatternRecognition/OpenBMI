session_name= 'brain_awareness_week_2012';

fprintf('\n\nWelcome to the study "%s"!\n\n', session_name);
startup_new_bbci_online;
addpath([BCI_DIR 'acquisition/setups/special_event/' session_name]);

%% Start BrainVisionn Recorder, load workspace and check triggers
system('start Recorder'); pause(1);
bvr_sendcommand('stoprecording');
if isequal(ACQ_PREFIX_LETTER, 'm'),
  bvr_sendcommand('loadworkspace', [session_name '_EasyCap']);
else
