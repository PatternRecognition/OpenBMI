...
desc= stimutil_readDescription('Brazil_imag_fbarrow');
...
fprintf('Record 1st run of feedback then press <EXIT> in the GUI.\n');
system(['matlab -r "' cmd 'setup_Brazil; matlab_control_gui(''D:\svn\bbci\online\setups\BCI_Brazil\cursor_adapt_pcovmean'', ''classifier'', [TODAY_DIR ''Lap_C3z4_bp_'' CLSTAG ''_v6'']);" &']);
bbci_bet_apply;
...
cmd= sprintf('CLSTAG= ''%s''; VP_CODE= ''%s''; ', CLSTAG, VP_CODE);
fprintf('Record 2nd run of feedback then press <EXIT> in the GUI.\n');
system(['matlab -r "' cmd 'setup_Brazil; matlab_control_gui(''D:\svn\bbci\online\setups\BCI_Brazil\cursor_adapt_pcovmean'');" &']);
bbci_bet_apply;
...
cmd= sprintf('CLSTAG= ''%s''; VP_CODE= ''%s''; ', CLSTAG, VP_CODE);
fprintf('Record 3rd runs of feedback then press <EXIT> in the GUI.\n');
system(['matlab -r "' cmd 'setup_Brazil; matlab_control_gui(''D:\svn\bbci\online\setups\BCI_Brazil\cursor_adapt_pmean'');" &']);
bbci_bet_apply;
