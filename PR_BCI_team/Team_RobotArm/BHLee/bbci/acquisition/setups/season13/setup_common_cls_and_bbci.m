...
global EEG_MAT_DIR DATA_DIR BCI_DIR EEG_RAW_DIR 
...
res_dir = [EEG_RAW_DIR 'subject_independent_classifiers/season13/'];
cnt_dir = [DATA_DIR 'results/vitalbci_season1/csp_patches/subject_independent_classifiers/'];

if isequal(center_str, '3c')
  suffix = ['C3z4_' opt.patch '_'];
else
  disp('decide how to convert center_str')
  keyboard
end
...
if isequal(opt.meth, 'lap')
  ...
  if isequal(opt.patch_selectPolicy, 'all')
    suffix_cnt = [suffix_cnt opt.patch_selectPolicy int2str(opt.patterns) 'csp_'];
  else
    suffix_cnt = [suffix_cnt int2str(opt.patterns) 'csp_' opt.patch_selectPolicy '_' opt.patch_score '_'];
  end
end
...
%   switch opt.meth
%     case 'lap'
%       proc.sneakin= {'patch_centers', cspp_opt.patch_centers};
...
%     case 'patches'
%       proc.sneakin= {'patch_centers', cspp_opt.patch_centers};
%       proc.train= ['[fv,W]= proc_cspp_auto(fv, ''patterns'', ' int2str(cspp_opt.patterns) ', ''require_complete_neighborhood'', ' ...
...
%     case 'patches'
%       [fv, W, usedPat, A, eig, Wall]= proc_cspp_auto(fv, cspp_opt);
%     case 'csp'
...
file = [cnt_dir 'common_classifier_' int2str(opt.nPatPerPatch) 'patPerPatch_' ...
  center_str '_' opt.patch '_' int2str(opt.patterns) 'csp_' opt.patch_selectPolicy '_' opt.patch_score '_' bandstr clstag '_' opt.meth];
load(file);

  if nBands>1
    for ib = 1:nBands
      [filt_b{ib} filt_a{ib}]= butter(5, band(ib,:)/fv.fs*2); %cspp_opt.fs*2);
    end
  else
    [filt_b filt_a]= butter(5, opt.band/fv.fs*2); %cspp_opt.fs*2);
  end
...
%% setup_opts
  ...
  setup_opts.patch = cspp_opt.patch;
  setup_opts.patch_centers = cspp_opt.patch_centers;
  setup_opts.patch_clab = cspp_opt.patch_clab;

%   setup_opts.usedPat = usedPat;
%   setup_opts.eig = eig;
%   setup_opts.A = A;
%   setup_opts.Wall = Wall;
  setup_opts.model = classy;
  setup_opts.clab = fv.origClab;

%% analyze  TODO: check in the adaptation pcovmean
  analyze.band = band;
  analyze.ival = ival;
  ...
  cont_proc.clab = fv.origClab;
  cont_proc.proc= {{@online_linearDerivation, W, fv.clab}, {@online_filt, filt_b, filt_a}};
  ...
  save([res_dir fileOut], 'classifier', 'setup_opts','analyze', 'feature', 'cont_proc');
      
  disp(['save in ' res_dir fileOut])

  clear classifier setup_opts analyze feature cont_proc
end
