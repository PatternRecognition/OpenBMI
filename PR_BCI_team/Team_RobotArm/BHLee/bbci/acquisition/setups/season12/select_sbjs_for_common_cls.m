...
if isequal(opt.meth, 'lap')
  ...
  if isequal(opt.patch_selectPolicy, 'all')
    suffix = [suffix opt.patch_selectPolicy int2str(opt.nPat) 'csp_'];
  else
    suffix = [suffix int2str(opt.nPat) 'csp_' opt.patch_selectPolicy '_' opt.patch_score '_'];
  end
end
  ...
  for ci= 1:size(nck,1),
    ...
    if length(mrk_cl.className) == 2
      ...
      switch opt.meth
        ...
        case 'patches'
          proc.train= ['[fv,W]= proc_cspp_auto(fv, ''nPat'', ' int2str(patches_opt.nPat) ', ''require_complete_neighborhood'', ' ...
