...
for ri= 1:nRuns,
 for ti= 1:length(tag_list),  
  ...
  bbci_cfy= [TODAY_DIR '/cls_ERD_24chan_mcc_' CLSTAG '_' VP_CODE];
  ...
  settings_bbci= {'start_marker', 210, ...
                  ...
                  'fb_port', 12345, ...
                   ...
                   'adaptation.adaptation_ival', [2000 4000], ...  % Vgl mit bbci.setup_opts.ival von VPfb
  ...
  settings_fb= {'classes', classes, ...
                ...
                'break_every', 30,...
                'cursor_visible',0 ...
                'duration_blank', 5000}; % break between trials
  ...
  setup.general.savestring= ['imag_fbarrow_pretrained_ERD_24chan_mcc_' CLSTAG];
...
bbci= bbci_default;
bbci.train_file= strcat(bbci.subdir, '/imag_fbarrow_pretrained_ERD_24chan_mcc_*');
...
bbci.setup_opts.model= {'RLDAshrink', 'scaling',1, 'store_means',1, 'store_invcov',1,'store_extinvcov',1};
bbci.setup_opts.clab= {'F3,4','FC5,1,2,6','C5-6','CCP5,3,4,6','CP3,z,4','P5,1,2,6'};

% Spezifisch für VPfb:
bbci.setup_opts.ival=[2000 4000];
bbci.setup_opts.band=[9 11];
...
adaptation_ival(1)=2000;
adaptation_ival(2)=4000;
...
for i =1:nRuns
  ...
  settings_fb= {'classes', classes, ...
    ...
    'trials_per_run', 60, ... 
    'break_every', 60,...
    'cursor_visible',1, ... % 1: cursor always visible, 0: cursor only visible at the end
    'pause_msg', [CLSTAG(1) ' vs. ' CLSTAG(2)],...
    'duration_blank', 5000}; % break between trials
  setup.graphic_player1.feedback_opt= overwrite_fields(setup.graphic_player1.feedback_opt, settings_fb);
  setup.control_player1.bbci= overwrite_fields(setup.control_player1.bbci, settings_bbci);
