...
% % For six conditions:
%conditions = {'Cube2D', 'Cube3D_medium','Cube3D_far','Scene_2D','Scene_medium','Scene_far'};
%marker_cond = [0 20 40 60 80 100];
% % Urspruenglich: marker_cond = [0 40 80 120]; % Hier aber 6, statt 4 conditions
% % Wuerde man auf 160 und 200 erhoehen wuerden die marker mit standard markers  ueberlappen
% % Marker >255 werden nicht angewandt!!! ppTrigger nutzt UINT8 "The values of a UINT8 range from 0 to 255"
% % Warum eigentlich immmer 40 Abstand? Eigentlich reicht ein Abstand von 14= 2(start&end) * 7 shifts

% For four conditions
conditions = {'Cube2D','Cube3D_far','Scene_2D','Scene_far'};
...
% Endgueltige shifts einfuegen!!! Bei sechs Konditionen: Moch zwei [...] [...]
% mehr
parameters= {
  [0:4:20,32]
  [0:4:20,32]
...
for c=1:numel(conditions)
  for i=1:numel(parameters{c})
    k=k+1;
    file_names{k} = sprintf(fmt_str, conditions{c}, conditions{c}, parameters{c}(i));
    % Zweimal i, da Start +0 und end +1
    file_markers(k) = marker_cond(c) + 2*i; 
    fprintf(fid, '%d, %s, %i\n', file_markers(k), conditions{c}, parameters{c}(i));
  end
end
fclose(fid);
