...
num_ival = 1
if NO_COUNT == 1
    results_file = 'classResults1IvalNoCount.mat'
    ...
    class_results = 'Count'
    results_file = ['classResults1Ival' num2str(num_ival) '.mat']
...
if DO_CLASSIFY == 1
    ...
    for f= 1:2:length(subject),    
        ...
        epo_vis= proc_selectIval(epo, ival');
        epo_vis= proc_meanAcrossTime(epo_vis);
        %  'LDA'
        [loss(ii,1), error(ii,1)]= xvalidation(epo_vis, 'LDA', opt_xv);
        %  'Fisher'
        [loss(ii,2), error(ii,2)]= xvalidation(epo_vis, 'FisherDiscriminant', opt_xv);
        %  'Shrink'
        [loss(ii,3), error(ii,3)]= xvalidation(epo_vis, 'RLDAshrink', opt_xv)

        
        
        % load tact data
        [cnt, mrk, mnt]= eegfile_loadMatlab(subject(f+1).path);
        [mk, rClab]= reject_varEventsAndChannels(cnt, mrk, disp_ival, ...
                                         'visualize', 0);
        mrk= mk;
        ...
        epo_tact= proc_selectIval(epo, ival');
        epo_tact= proc_meanAcrossTime(epo_tact);
        %  'LDA'
        [loss(ii,4), error(ii,4)]= xvalidation(epo_tact, 'LDA', opt_xv);
        %  'Fisher'
        [loss(ii,5), error(ii,5)]= xvalidation(epo_tact, 'FisherDiscriminant', opt_xv);
        %  'Shrink
        [loss(ii,6), error(ii,6)]= xvalidation(epo_tact, 'RLDAshrink', opt_xv)
        ii = ii + 1
    end

    
    save([EEG_MAT_DIR results_file], 'loss', '-ASCII')
    save([EEG_MAT_DIR results_file '_error'], 'error', '-ASCII')
...
if DO_IMAGES == 1
 ...
 loss1= load([EEG_MAT_DIR 'classResults1Ival1.mat'], '-ASCII')
 error1= load([EEG_MAT_DIR 'classResults1Ival1.mat_error'], '-ASCII')
 ...
 hold all
 bar(loss,0.2)
 errorbar(loss,error,'r','linestyle', 'none', 'linewidth', 2)
 xLab = {'Visual Stimulus';'Tactile Stimulus';}
 set(gca,'xticklabel',xLab)
 set(gca,'XTick',[1 2]) 
 ylim([0 0.5])
 ylabel('Average Classification Error % with error bars')
 xlabel('Stimulus Method')   
  
 hold off
 
savename=fullfile(LATEX_DIR, 'avg_stimulus_error')
...
loss2= load([EEG_MAT_DIR 'classResults1Ival2.mat'], '-ASCII')
loss3= load([EEG_MAT_DIR 'classResults1Ival3.mat'], '-ASCII')
loss4= load([EEG_MAT_DIR 'classResults1Ival4.mat'], '-ASCII')
loss5= load([EEG_MAT_DIR 'classResults1Ival5.mat'], '-ASCII')
loss6= load([EEG_MAT_DIR 'classResults1Ival6.mat'], '-ASCII')
...
g = cat(1,loss1Avg,loss2Avg,loss3Avg,loss4Avg,loss5Avg,loss6Avg)
