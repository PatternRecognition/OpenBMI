...
if isempty(state),
  state.counter= zeros(1, 10);
  state.output= zeros(1, 10);
end

this_cue= 1 + mod(event.desc-1, 10);
...
if sum(state.counter) >= opt.nClasses*opt.nSequences,
  idx= find(state.counter>0);  % regard non-void classes only
  state.output(idx)= state.output(idx) ./ state.counter(idx);
  [max_score, selected_class]= min(state.output(idx));
  packet= {'i:cl_output', idx(selected_class)};
  state.counter(:)= 0;
  state.output(:)= 0;
else
  packet= [];
