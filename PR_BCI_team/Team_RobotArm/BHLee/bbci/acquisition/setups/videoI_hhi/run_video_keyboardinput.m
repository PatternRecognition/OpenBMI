...
for n = 1:length(stim_seq)
  count = count.oneup();
  vid = vids{stim_seq(n)};
  QLevel = [QLevel int2str(vid(7:end-4))];     %%% TODO %%%
  responded = 0;
  % start video
  cmd = ['cmd /C "C: & cd ' VLC_DIR ' & vlc ' VLC_OPTS ' ' videodir vid];
  dos(cmd);
  while ~responded
    s = stimutil_waitForMarker('stopmarkers',{R_LQ, R_HQ}, ...
      'bv_bbciclose',0, 'state',state);
...
for v = (block-1)*N_perBlock+1:N
    ...
    ppTrigger(str2num(videos{stim_seq(v)}(7:end-4)));     %%% TODO %%%
    % show video
    cmd= ['cmd /C "C: & cd ' VLC_DIR ' & vlc ' VLC_OPTS ' ' vidname];
    dos(cmd);
    % wait for user input
