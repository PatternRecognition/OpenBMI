...
%          acquire_bv(acquire_state);
    ...
    [out,out_aa,mrk_from_condition]= ...
        getClassifier('apply',size(data.x,1)*1000/bbci.fs,cls);
    
    %%% if necessary perform mrk_out %%%
    mrkOut= performMarkerOutput('apply', ...
                        marker_output,size(data.x,1)*1000/bbci.fs,timestamp);
...
%%% log out %%%
    ...
    if isfield(bbci,'feedback') & ~isempty(bbci.feedback),
      if nargin(['bbci_bet_feedbacks_' bbci.feedback])==3,
    ...
    if isempty(bbci.fb_machine)
      ...
      switch(general_port_fields.feedback_receiver),
       ...
       case 'pyff',
        if ~isnan(udp(1)),
          if isinteger(udp),
            send_xmlcmd_udp('i:controlnumber', controlnumber, ...
                            'timestamp', timestamp, ...
                            'i:cl_output', udp(1));
          else
    ...
    if ~isempty(intersect(toe, bbci.start_marker)),
      fprintf('[bbci_bet_apply:] Start marker %d received.\n', toe);
      start_marker_received= 1;
    end
    if ~isempty(intersect(toe, bbci.quit_marker)) & start_marker_received,
      fprintf('[bbci_bet_apply:] Quit marker %d received.\n', toe);
      loop= false;
      run= false;
      pause(5);
