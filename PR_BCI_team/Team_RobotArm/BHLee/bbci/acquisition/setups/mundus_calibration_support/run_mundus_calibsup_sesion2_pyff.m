...
opt = [];
opt.dir = 'D:\development\pyff\src'
...
%% TEST FES
RAW_ETH = 1; 
[stim, opt] = setup_mundus_calibsup_image_arrow({'right','foot'}, 5, 2000, 3000);
...
bbci.setup='cspauto';
bbci.train_file= strcat(bbci.subdir, '/MI_cb*');
...
% FEEDBACK
%% ** Startup pyff **
system(['cmd /C "D: & cd \development\pyff\src & python FeedbackController.py --port=0x' dec2hex(IO_ADDR) ' --nogui -l debug -p brainvisionrecorderplugin --additional-feedback-path=D:\development\bbci\python\pyff\src\Feedbacks\SMR_NeuroFeedback" &']);
...
cmd_bbci= ['dbstop if error; global BBCI_VERBOSE; BBCI_VERBOSE=1; dbstop in get_data_udp at 58; bbci_bet_apply(''' bbci_cfy ''');'];
