...
%-newblock
%% - CUED TAPPING Test Run
% setup_season9_cued_tapping;
% fprintf('Press <RETURN> when ready to start ''cued tapping'' TEST RUN.\n');
% pause
...
% %% - CUED TAPPING Run 1
% setup_season9_cued_tapping;
% fprintf('Press <RETURN> when ready to start ''cued tapping'' RECORDING of Run 1.\n');
% pause
...
system(['matlab -r "' cmd 'setup_season9; matlab_control_gui(''season9/cursor_adapt_pcovmean'', ''classifier'', [TODAY_DIR ''Lap_C34_bp_'' CLSTAG]);" &']);
bbci_bet_apply;
...
%% - CUED TAPPING Run 2
setup_season9_cued_tapping;
fprintf('Press <RETURN> when ready to start ''cued tapping'' RECORDING of Run 2.\n');
pause
stim_visualCues(stim, opt);
...
%% - CUED TAPPING Run 3
setup_season9_cued_tapping;
fprintf('Press <RETURN> when ready to start ''cued tapping'' RECORDING of Run 3.\n');
pause
stim_visualCues(stim, opt);
...
%-newblock
%% - BBCI adaptive Feedback, CSP-based classifier, pmean adaptation RUN 2
desc= stimutil_readDescription('season9_imag_fbarrow');
h_desc= stimutil_showDescription(desc, 'clf',1, 'waitfor',0);
fprintf('Press <RETURN> when ready to start the next task.\n');
pause
...
%-newblock
%% - BBCI adaptive Feedback, CSP-based classifier, pmean adaptation RUN 3
desc= stimutil_readDescription('season9_imag_fbarrow');
h_desc= stimutil_showDescription(desc, 'clf',1, 'waitfor',0);
fprintf('Press <RETURN> when ready to start the next task.\n');
pause
