function sound_presentation(varargin)
global VP_SCREEN VP_CODE BCI_DIR TODAY_DIR VLC_DIR SOUND_DIR STIM_DIR DATA_DIR
% presents music
...
% 254: end
% 11 for beginning of excerpt
% 12 for end of excerpt
% 
% 
...
STIM_DIR= [DATA_DIR 'music\sound_discretization\irene_pilot\'];
...
playlist=textread([BCI_DIR 'acquisition/stimulation/sound_discretization/' opt.playlist],'%s');
...
if ~opt.joystick
    fprintf(1,'no ratings are recorded!\n')
        ...
        p = audioplayer(y, f); 
        ppTrigger(11);%trigger for begin of excerpt
        %start playing
        play(p,f)
        while(isplaying(p))
            pause(0.02)
        ...
        d1=input('Wieviele verschiedene Typen von Sounds haben Sie soeben gehoert?')
        ppTrigger(d1)
        d2=input('Falls Sie einen Zyklus gehört haben, wieviele events haben Sie gehört?')
        ppTrigger(d2)
        pause(2)
        clear y
    end
    ...
    for i=1:nr_of_excerpts
        ...
        j_val=jst;
        ppTrigger(12);%trigger for endtrigger for begin of excerpt of excerpt
