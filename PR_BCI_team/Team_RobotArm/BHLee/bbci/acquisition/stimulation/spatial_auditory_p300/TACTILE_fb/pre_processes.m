...
if ~isfield(opt.procVar, 'handle_background'),
    if isfield(opt, 'tact_duration') && opt.tact_duration,
      send_trigger_vest('init', opt.tact_ip, opt.tact_port);
      send_trigger_vest('duration', opt.tact_duration);
    end
    opt.procVar.handle_background = stimutil_initFigure(opt);
    [opt.procVar.handle_cross, opt.procVar.handle_loc, dum, opt.procVar.handle_tick]= stimutil_fixationCrossandLocations(opt, 'angle_offset', 30, 'tick_marks', 1);
    if opt.show_result,
      opt.procVar.scores = zeros(1,2);
      xlimpos = get(gca, 'Xlim');
      ylimpos = get(gca, 'Ylim');
      verPos = ylimpos(2)-.2;
      horPos = mean([xlimpos(2), xlimpos(1)]);
      opt.procVar.handle_scores_lab(1) = text(horPos-.5, verPos, 'Correct:', 'color', [1 1 1], 'FontSize', 32, 'HorizontalAlignment', 'center', 'visible', 'off');
      opt.procVar.handle_scores_lab(2) = text(horPos+.2, verPos, 'Fout:', 'color', [1 1 1], 'FontSize', 32, 'HorizontalAlignment', 'center', 'visible', 'off');
      opt.procVar.handle_scores(1) = text(horPos-.25, verPos, '0', 'color', [1 1 1], 'FontSize', 32, 'HorizontalAlignment', 'center', 'visible', 'off');
      opt.procVar.handle_scores(2) = text(horPos+.37, verPos, '0', 'color', [1 1 1], 'FontSize', 32, 'HorizontalAlignment', 'center', 'visible', 'off');      
      opt.procVar.handles_all = [opt.procVar.handle_loc opt.procVar.handle_cross(1) opt.procVar.handle_cross(2), opt.procVar.handle_tick{:}, opt.procVar.handle_scores, opt.procVar.handle_scores_lab];
    else
      opt.procVar.handles_all = [opt.procVar.handle_loc opt.procVar.handle_cross(1) opt.procVar.handle_cross(2), opt.procVar.handle_tick{:}];
    end
    if isfield(opt, 'requireResponse') && opt.requireResponse,
      opt.procVar.response_text = text(0, .1, 'Hoeveel tonen heb je geteld?: ','color', [1 1 1], 'fontsize', 30, 'HorizontalAlignment', 'center', 'Visible', 'off');
      opt.procVar.response_field = text(0, -.1, '','color', [1 1 1], 'fontsize', 30, 'HorizontalAlignment', 'center', 'Visible', 'off');
      KbName('UnifyKeyNames');
    end
end

set([opt.procVar.handle_tick{:}], 'visible', 'off');
