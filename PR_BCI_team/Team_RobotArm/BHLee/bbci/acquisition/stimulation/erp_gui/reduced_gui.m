function varargout = reduced_gui(varargin)
...
% Last Modified by GUIDE v2.5 12-Sep-2011 18:49:45

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @reduced_gui_OpeningFcn, ...
    'gui_OutputFcn',  @reduced_gui_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
function reduced_gui_OpeningFcn(hObject, eventdata, handles, varargin)
...
function varargout = reduced_gui_OutputFcn(hObject, eventdata, handles)
...
hdr = eegfile_readBVheader(sel_files(get(handles.file_list_box, 'value')));
assignin('base', 'original_fs', hdr.fs);
...
evalin('base', 'bbci.train_file = tmp;');
...
elseif isempty(strmatch('VP', get(handles.vp_code_box, 'string'))) && ~isempty(get(handles.vp_code_box, 'string')),
    ...
    if get(handles.amp_button_gtec, 'value')
        assignin('base', 'acquire_func', @acquire_sigserv);
    else
        assignin('base', 'acquire_func', @acquire_bv);
    end
    evalin('base',['p300gui_setup_online_' studies{get(handles.study_box,'value')}]);
...
for i = 1:length(fh),
    if ~strcmp(get(fh(i), 'name'), 'reduced_gui'),
        close(fh(i));
    end
end
...
if check_if_initialized(handles) && check_if_data_loaded(handles),
    ...
    if sel_class > 1,
        ...
        try
            ...
            add_to_message_box(handles, sprintf('Normalized performance: %2.1f%%', xval_result(3)));
        catch
            error = lasterror;
            disp(error.message);
            set_button_state(handles, 'on', {''});
...
if check_if_initialized(handles),
    ...
    if ~strcmp(classifiers{sel_class}, 'Initialize first') && ~isempty(classifiers{sel_class}) && ~strcmp(classifiers{sel_class}, 'No matching classifier found.'),
        ...
        if get(handles.amp_button_gtec, 'value'),
            amp_cmd = 'acquire_func = @acquire_sigserv; ';
        else
            amp_cmd = 'acquire_func = @acquire_bv; ';
        end
        ...
        system(['matlab -nosplash --minimize -nojvm -r "' amp_cmd cmd_init cmd_bbci '; exit &']);
    end
end
...
if check_if_initialized(handles) && check_if_data_loaded(handles),
    ...
    sel_clab = get(handles.channel_names_listbox, 'value');
    clab_name = get(handles.channel_names_listbox, 'string');
    
    template = 'D:\Dropbox\Dropbox\FSL_Study_Root\settings\template_sigserv_config.xml';
    ...
    chans = '';
    for i = 1:length(sel_clab),
        chans = [chans sprintf('<ch nr="%i" name="%s" type="eeg" />\r', i, clab_name{i})];
    end
    
    str = sprintf(str, VP_CODE, length(sel_clab), chans);
...
add_to_message_box(handles, 'Switching to g.Tec. Please start the signal server through this gui.');
...
assignin('base', 'acquire_func', @acquire_bv);
% assignin('base', 'tmp', 1000);
% evalin('base', 'bbci.original_fs = tmp; clear tmp;');
add_to_message_box(handles, 'Switching to BrainAmp. Please start the recorder software by hand.');
...
if get(hObject, 'Value'),
    set([handles.channel_select_panel, handles.images_panel], 'Visible', 'on');
else
    set([handles.channel_select_panel, handles.images_panel], 'Visible', 'off');
end
