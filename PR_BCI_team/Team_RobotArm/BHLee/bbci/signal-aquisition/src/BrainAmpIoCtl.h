...
//; Brain Amp Setup.
{
	...
	unsigned short 	nHoldValue;			// Hold value for digital input

	// Version 1.01.0003 and higher
	// The following tables are based on logical channel positions.
	unsigned char	n250Hertz[256];		// Low pass 250 Hz (0 = 1000Hz)
	unsigned char	nResolution[256];	// 0 = 100 nV, 1 = 500 nV, 2 = 10 µV, 3 = 152.6 µV
	unsigned char	nDCCoupling[256];	// DC coupling (0 = AC)
	unsigned char	nLowImpedance;		// Low impedance i.e. 10 MOhm, (0 = > 100MOhm)
};
...
{
	unsigned short	nWaveForm;			// 0 = ramp, 1 = triangle, 2 = square, 3 = sine wave
	unsigned long	nFrequency;			// Frequency in millihertz.
};
