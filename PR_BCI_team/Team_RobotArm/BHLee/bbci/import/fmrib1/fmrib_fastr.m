...
%   FMRI artifact Slice Template Removal (FASTR) [Nizy04] and uses optiman
%   basis sets (OBS) to reduce the residuals.  
%
%   This subtraction algorithm is based on the principles outlined by 
%   [Allen00] with few improvements and modifications. This program 
...
%
%   USAGE:
%   EEG=fmrib_fastr(EEG,lpf,L,window,Trigs,strig,tc_chk,Volumes,Slices,varargin);
...
%
% [Niazy04] Improved FMRI Artifact Reduction from Simultaneously
%   Acquired EEG Data using Slice Dependant Template Matching, Rami K. Niazy,
%   Giandomenico Iannetti, Christian Beckmann, Michael Brady, Stephen Smith.
%   Proc. 12th ISMRM (2004), Kyoto, Japan.
%
...
Idata=zeros(1,(fs*SecT+2*pad_sec)*L);
Iorig=zeros(1,(fs*SecT+2*pad_sec)*L);
INoise=zeros(1,(fs*SecT+2*pad_sec)*L);
fNoise=zeros(1,(fs*SecT+2*pad_sec));
Noise=zeros(1,n);
fcleanEEG=zeros(1,(fs*SecT+2*pad_sec));
cleanEEG=zeros(1,n);
tmpd=zeros(n,1);
d=zeros(mANC,1);
refs=zeros(mANC,1);
...
while sec<=sections
    ...
    if sec==1 & sections > 1
        rempeaks=peaks((d1-1)+sec*fs*SecT+1:d2);
        remmarker=find(rempeaks==1);
        if length(remmarker) > 3*hwinl
            secpeaks=peaks(d1:(d1-1)+sec*SecT*fs+pad_sec);
        else
    ...
    elseif sec==1 & sections==1
        secpeaks=peaks(d1:d2);
    elseif sec==sections
        secpeaks=peaks((d1-1)+(sec-1)*fs*SecT+1-pad_sec:d2);
    else
        rempeaks=peaks((d1-1)+sec*fs*SecT+1-pad_sec:d2);
        remmarker=find(rempeaks==1);
        if length(remmarker) > 3*hwinl
            secpeaks=peaks((d1-1)+(sec-1)*fs*SecT+1-pad_sec:...
                (d1-1)+sec*fs*SecT+pad_sec);
        else
            secpeaks=peaks((d1-1)+(sec-1)*fs*SecT+1-pad_sec:d2);
            sec=sections;
        end
...
for c=1:m
    ...
    for sec=1:sections
        ...
        if L > 1
            ...
            if sec==1 & sections > 1
                Idata=tmpdata(d1:(d1c-1)+secl(sec)+pad_sec);
        ...
        for s=starts:lasts
         ...
         if isempty(intersect(exc,c)) & npc~=0
            ...
            [apc,ascore,asvar]=pca_calc(pcamat');
            ...
            if sec==1
                if ischar(npc)
                    ...
                    CUMVARTH_PC=find(cumsum(oev)>TH_CUMVAR,1);
                    VAREXPTH_PC=find(oev<TH_VAREXP,1)-1;
            ...
            papc=[ascore(:,1:pcs) ones(pre_peak+max_postpeak+1,1)];
            ...
            papc=ones(pre_peak+max_postpeak+1,1);
            for s=starts:lasts
                if s==1
                    if ~STARTFLAG
                       fitted_res(secmarker(s)-pre_peak:secmarker(s)+max_postpeak)=...
