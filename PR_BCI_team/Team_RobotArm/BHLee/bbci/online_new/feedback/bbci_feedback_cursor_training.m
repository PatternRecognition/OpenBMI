function opt = bbci_feedback_cursor_training(opt, ctrl);
%BBCI_FEEDBACK_CURSOR_TRAINING - Cursor Movement Training with Arrow Cues
%
%Synopsis:
% OPT= bbci_feedback_cursor_training(OPT, CTRL)
%
%Arguments:
...
if opt.reset,
  ...
  [HH, cfd]= bbci_feedback_cursor_training_init(opt);
...
if state==0,
  ...
  if memo.timer+memo.timeeps >= opt.countdown,
    ...
    if memo.modus==0,      %% during break inbetween run
      ...
      if mod(opt.trials_per_run,nClasses)>0 && isempty(opt.seq_mod),
        warning('parameter ''break_every'' should be multiple of nClasses.');
      end
      if isempty(opt.seq_mod),
        seq_mod= 0;
      else
        seq_mod= opt.seq_mod;
      end
      ...
      for bb= 1:nBlocks,
        ...
        seq_order= circshift((1:nClasses)', seq_mod);
        seq_block= seq_order(bseq(pp));
        seq= cat(2, seq, seq_block');
        seq_mod= seq_mod + 1;
      end
...
if state==5,  %% move cursor until target was hit or time-out
  ...
  evidence_in_favor_of_cue= classifier_factor .* ctrl;
  if max(abs(evidence_in_favor_of_cue))==max(evidence_in_favor_of_cue),
    if opt.proportional_control,
      ctrl= min(opt.input_range(2), max(evidence_in_favor_of_cue));
    else
      ctrl= opt.input_range(2);
    end
  else
    % negative feedback
    if opt.proportional_control,
      punish= min(evidence_in_favor_of_cue) + ...
              max([evidence_in_favor_of_cue; 0]);
      ctrl= max(opt.input_range(1), opt.gain_error*punish);
    else
      ctrl= opt.input_range(1);
    end
  end
  if ctrl>0,
    % positive feedback
