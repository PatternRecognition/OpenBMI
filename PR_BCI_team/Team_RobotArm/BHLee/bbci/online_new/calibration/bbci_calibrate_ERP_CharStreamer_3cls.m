...
if isfield(bbci.calibrate.default_settings, 'band')
    if ~isempty(bbci.calibrate.default_settings.band)
        [filt_b,filt_a]= butter(5, bbci.calibrate.default_settings.band/data.cnt.fs*2);
...
BC_result.mrk= mrk_defineClasses(data.mrk, bbci.calibrate.marker_param{ix_cls});
stimBeforeTarget = union(union(union(find(BC_result.mrk.y(1,:))+1,  find(BC_result.mrk.y(1,:))-1 ), union(find(BC_result.mrk.y(1,:))+2,  find(BC_result.mrk.y(1,:))-2 )), union(find(BC_result.mrk.y(1,:))+3,  find(BC_result.mrk.y(1,:))-3 ));
stimBeforeTarget(stimBeforeTarget < 1) = [];
stimBeforeTarget(stimBeforeTarget > size(BC_result.mrk.y,2)) = [];
BC_result.mrk = mrk_selectEvents(BC_result.mrk, union(find(BC_result.mrk.y(1,:) | (BC_result.mrk.y(2,:) & rand(1, size(BC_result.mrk.y,2)) <.15 )), stimBeforeTarget ));
...
if isfield(bbci.calibrate.default_settings, 'band') %set filter if required!
    if ~isempty(bbci.calibrate.default_settings.band)
        online_sampl_rate = 50 %bbci.source.acquire_param{1}.fs; %extract sampling rate from bbci-strukt 
        [filt_b,filt_a]= butter(5, bbci.calibrate.default_settings.band/online_sampl_rate*2);
        bbci.signal.proc= {{@online_filt, filt_b, filt_a}};
    end
end


if isempty(bbci.calibrate.default_settings.ref_ival)% no baselining
        bbci.feature(ix_cls).proc= {{@proc_jumpingMeans, BC_result.cfy_ival}}; 
        min_t = min(BC_result.cfy_ival(:));
else
    bbci.feature(ix_cls).proc= {{@proc_baseline, BC_result.ref_ival, 'beginning_exact'}, ...
                    {@proc_jumpingMeans, BC_result.cfy_ival}};
    min_t =  min(BC_result.ref_ival(1), min(BC_result.cfy_ival(:)));
end
bbci.feature(ix_cls).ival= [min_t BC_result.cfy_ival(end)];
bbci.feature(ix_cls).signal= 1; %take the same signal stream

% there is control_per_stimulus
  bbci.control(ix_cls).fcn= @bbci_control_ERP_Speller_binary;
...
bbci.control(ix_cls).param= {struct('nClasses',opt.nClasses, ...
                            'nSequences',BC_result.nSequences, ...
                            'mrk2feedback_fcn', mrk2feedback_fcn)};
                        
bbci.control (ix_cls).condition.marker= opt.cue_markers{ix_cls};


fv= cntToEpo(data.cnt, BC_result.mrk, bbci.feature(ix_cls).ival, ...
             'clab',bbci.signal.clab);
fv= bbci_calibrate_evalFeature(fv, bbci.feature(ix_cls));
...
bbci_log_write(data.log.fid, ['cls ' num2str(ix_cls) ': ROC-loss: %.1f%%  |  ' ...
                    'Correct Hits: %.1f%%,  Correct Miss: %.1f%% \n'], ...
               100*loss, 100*me(1,1), 100*me(2,2));

data.feature= fv;
data.result= BC_result;
data.result.rocloss{ix_cls} = 100*loss;
data.result.me{ix_cls} = me;

clear data.result data.cnt fv epo
pack;
drawnow;

pause(5) %pause such that the Versuchsleiter can doublecheck the images
bbci.feedback(ix_cls).control = ix_cls;
bbci.feedback(ix_cls).receiver = 'pyff';
end
