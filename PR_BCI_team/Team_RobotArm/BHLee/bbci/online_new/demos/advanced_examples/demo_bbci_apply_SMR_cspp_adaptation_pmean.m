...
S=  getClassifierLog('VPjj_08_06_10');
...
bbci.save_name= strcat(TMP_DIR, 'bbci_classifier_cspp_auto_csp48_pmean');
bbci.withgraphics= 0;
bbci_bet_prepare
bbci_bet_analyze
...
bbci.adaptation.param= {struct('ival',adaptation_ival)};
clear epo_flt
bbci.adaptation.filename= strcat(TMP_DIR, 'bbci_classifier_pmean');
% bbci.adaptation.log.output= 'screen';
bbci.adaptation.load_classifier= 1;

[cnt, mrk_orig]= eegfile_readBV([eeg_file '03'],'fs', bbci.fs, 'clab', clab_load);
...
bbci.log.file= strcat(TMP_DIR,'log_cspp_pmean');
