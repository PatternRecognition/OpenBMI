...
for sub = 1:length(filelist)
    ...
    % Data load - MI
    [cntGrasp,mrkGrasp,mntGrasp]=eegfile_loadMatlab([dd 'MI' '\' fs{fsldx} '\' filelist{sub} '_multigrasp_' 'MI']);
    [cntTwist,mrkTwist,mntTwist]=eegfile_loadMatlab([dd 'MI' '\' fs{fsldx} '\' filelist{sub} '_twist_' 'MI']);
    ...
    for filt = 1:length(filterBank)
        ...
        str3=char('.mat');
%         str4=char();
        save(['C:\Users\Doyeunlee\Desktop\AAAI_20190717\Dataconstruction_filter\MI_train/',num2str(sub),filterBank{filt},str3],'trainX','trainY');
        save(['C:\Users\Doyeunlee\Desktop\AAAI_20190717\Dataconstruction_filter\MI_test/',num2str(sub),filterBank{filt},str3],'testX','testY');
    end % filterBank
end % subject
